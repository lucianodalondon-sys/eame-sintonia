...
**Próximo passo autorizado (2026-09-16):** REVISÃO FINAL DA SOURCES e, se o candidato continuar 0 atrás do trunk, FAST-FORWARD de `claude/it-sources-atlas-v1` para `claude/it-trunk-v1`. Fotografia medida ao escrever isto: `CANDIDATE_HEAD = 3bdb34badd26ffbb630b4a35b3f2d13b8730dead` · `TRUNK_HEAD = 8ad9d9a263a0557040642722459373e6dae3f396` · 8 à frente / 0 atrás (o commit que corrige este ponteiro fica 9 à frente). Depois da integração: recalibrar o System Map pela cadeia canónica (`correr_a_cadeia.py REGERAR → VALIDAR → commit → PORTOES_POS_COMMIT`). ⚠️ A integração **ainda não aconteceu** — CAN DO ≠ DID DO. (O ponteiro anterior, «PREPARAR ADAMA REFERENCE» com trunk `f888b363` / Reference `91998964` / Sources `2f0863d1`, ficou cumprido pelos commits `db8de065`…`3bdb34ba` desta branch.)
