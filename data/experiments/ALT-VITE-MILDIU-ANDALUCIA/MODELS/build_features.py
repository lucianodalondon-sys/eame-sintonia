#!/usr/bin/env python3
"""
Build CUTOFF-RESPECTING seasonal predictors for Andalucian vine mildiu.

Same machinery as the Veneto pilot, re-timed to Andalucian phenology: bud break in March
rather than April, so the target season is 1 Mar - 31 Aug and the pre-season cutoff has to
fall in FEBRUARY, not March. Copying the Veneto windows unchanged would have let February
and early March of the target season into the predictors — a leak that the assertion below
would have caught, but which is better not to write in the first place.
...
def blocks_for(Y, regime):
    ...
    if regime == "CUTOFF_B_TRUE_12M":
        cutoff = D(Y - 1, 8, 31)
        blocks = [
            ("prevseason",   D(Y - 1, 3, 1),  D(Y - 1, 8, 31)),   # season Y-1 (Mar-Aug)
            ("prevwinter",   D(Y - 2, 12, 1), D(Y - 1, 1, 31)),   # winter before season Y-1
            ("prevautumn",   D(Y - 2, 9, 1),  D(Y - 2, 11, 30)),  # autumn Y-2
            ("prev2season",  D(Y - 2, 3, 1),  D(Y - 2, 8, 31)),   # season Y-2
        ]
    elif regime == "CUTOFF_A_PRESEASON":
        cutoff = D(Y, 1, 31)                                      # BEFORE March bud break
        blocks = [
            ("prevseason",   D(Y - 1, 3, 1),  D(Y - 1, 8, 31)),
            ("autumn",       D(Y - 1, 9, 1),  D(Y - 1, 11, 30)),  # autumn just past
            ("winter",       D(Y - 1, 12, 1), cutoff),            # Dec + Jan only
            ("prev2season",  D(Y - 2, 3, 1),  D(Y - 2, 8, 31)),
        ]
    else:
...
def target_season(Y):
    # Andalucian vine season: bud break March, harvest August. Mildiu is active Mar-Jun.
    return D(Y, 2, 1), D(Y, 9, 30)
...
if __name__ == "__main__":
    ...
    YEARS = list(range(1992, 2026))  # labels exist only for 2006-2025
    out = {}
    ...
    print("\nLABELLED WINDOW ONLY (2006-2025); full table has %d seasons" % len(r))
    print("YEAR  " + "  ".join(f"{k[:18]:>18s}" for k in ks))
    for Y in sorted(y for y in r if y >= 2006):
