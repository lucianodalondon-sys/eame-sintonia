...
def main():
    ...
    # UMA CORRIDA SEM CREDENCIAL NAO APAGA UMA PROVA JA FEITA.
    # DEST e um artefacto VERSIONADO. A versao anterior escrevia-o sempre, tambem
    # quando a corrida nao provou nada — bastava correr o script sem APIFY_TOKEN_POOL
    # para o registo commitado (READ_MODE REREAD_PRESERVED_RAW, 4 leis, captured_at da
    # coleta) ser substituido por um registo degradado com a data de hoje. Medido na
    # revisao adversarial do PASSO 03: a arvore ficava suja e a evidencia, perdida.
    DEGRADADO = ('APIFY_ENV_MISSING', 'RAW_NOT_PRESENT')
    if out.get('STATE') in DEGRADADO and os.path.exists(DEST):
        print('NAO_ESCREVI = %s' % os.path.relpath(DEST, ROOT))
        print('POR_QUE     = STATE=%s nao prova nada, e ja existe registo no disco. '
              'Sobrescrever seria trocar prova por ausencia de prova.' % out.get('STATE'))
    else:
