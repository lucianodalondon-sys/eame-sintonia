...
def main():
    ...
    for x in rot:
        ...
        m[(x["FACT_KIND"], r["fact_kind"])] += 1
        linhas.append({"ID": x["ID"], "ROTULO": x["FACT_KIND"], "CLASSIFICADOR": r["fact_kind"],
                       "BASE": r["fact_kind_basis"][:240]})
    por_tipo = {}
    for k in TF.TIPOS:
        if k == TF.NAO_SEI:
            continue
    ...
    out = {"DATASET": "MEDICAO-FATO-%s-V1" % nome, "MEDIDO_DENTRO_DA_AMOSTRA": not cega, "ITENS": len(rot),
           ...
           "LINHAS": linhas}
    (AQUI / ("MEDICAO-FATO-%s-V1.json" % nome)).write_text(json.dumps(out, ensure_ascii=False, indent=1) + "\n",
                                                          encoding="utf-8")
