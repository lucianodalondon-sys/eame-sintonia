...
MUTANTES = [
    ...
    ("M28_SEM_A_MARCA_DA_LEI", "DA-7: reprova se «oggi, lunedì» com publicacao so-dia perder PUBLISHED_AT_COM_PROVA (a lei reprova)",
     [("            fact_time_basis = PUBLICADO_COM_PROVA if fact_time == pub.isoformat() else RELATIVA\n",
       "            fact_time_basis = RELATIVA\n")]),
    ("M29_MARCA_DA_LEI_SEMPRE", "DA-7: reprova se a conta que da OUTRO dia disser PUBLISHED_AT_COM_PROVA",
     [("            fact_time_basis = PUBLICADO_COM_PROVA if fact_time == pub.isoformat() else RELATIVA\n",
       "            fact_time_basis = PUBLICADO_COM_PROVA\n")]),
    ("M30_EVIDENCIA_DA_CONTA_PERDIDA", "DA-7: reprova se a expressao e o trecho da conta se perderem",
     [("            fact_time_evidencia = (\"%s · %s · «%s» contado", "            fact_time_evidencia = NAO_SE_APLICA or (\"%s · %s · «%s» contado")]),
