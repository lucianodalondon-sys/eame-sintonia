...
MUTANTES = [
    ...
    ("M33_RELATIVA_SO_COM_ANCORA_DE_TEMPO", "ensaio: reprova se «registrato la settimana scorsa» voltar a dar lugar sem data",
     [('x["KIND"] = CAMPO if _relativa_presa_ao_campo(frase)', 'x["KIND"] = CAMPO if _presa_ao_campo(frase)')]),
    ("M34_BOLLETTINO_PRENDE_A_RELATIVA", "ensaio: reprova se «il prossimo bollettino … la prossima settimana» virar data",
     [('_ANCORAS_DE_LUGAR_PARA_O_TEMPO = tuple(a for a in FL.ANCORAS_POSITIVAS if "bollettino" not in a[0])',
       '_ANCORAS_DE_LUGAR_PARA_O_TEMPO = tuple(FL.ANCORAS_POSITIVAS)')]),
    ("M35_CONSELHO_E_FACTO", "ensaio: reprova se «eseguire … in luglio» voltar a ser tempo do facto",
     [("        e_conselho = bool(_RE_RECOMENDACAO.search(ev)) or", "        e_conselho = False and bool(_RE_RECOMENDACAO.search(ev)) or")]),
    ("M36_PEDACO_DE_CONSELHO_E_FACTO", "ensaio: reprova se o pedaco repetido de um conselho voltar a ser facto",
     [("(len(pedaco) >= 30 and any(pedaco in c for c in conselhos))", "False")]),
    ("M37_CONSELHO_RELATIVO_CONTA", "ensaio: reprova se «si consiglia … la prossima settimana» virar data",
     [("        elif _RE_RECOMENDACAO.search(frase):\n", "        elif False:\n")]),
    ("M38_FRASE_CORTADA_A_300", "ensaio: reprova se a palavra de evento depois das 300 letras voltar a perder-se",
     [('        frase = _frase_em(c, i) if i >= 0 else r["EVIDENCE"]\n', '        frase = r["EVIDENCE"]\n')]),
    ("M39_LOJA_NAO_E_MERCADO", "ensaio: reprova se «14 punti vendita» a Firenze deixar de ser MERCADO",
     [('_RE_LOJA = re.compile(r"(?<![0-9a-zà-ÿ])(?:punt', '_RE_LOJA = re.compile(r"(?!x)x(?:punt')]),
    ("M40_PAIS_PROMOVIDO", "ensaio: reprova se «focus sull'Italia» voltar a ser lugar de evento",
     [('        if r["PRECISION"] == "COUNTRY" or (', '        if False or (')]),
    ("M41_AMERICA_LATINA_E_LATINA", "ensaio: reprova se «dall'America Latina» voltar a ser a provincia de Latina",
     [('_NOME_MAIOR = re.compile(r"America\\s+Latina|Latinoamerica", re.I)', '_NOME_MAIOR = re.compile(r"(?!x)x")')]),
