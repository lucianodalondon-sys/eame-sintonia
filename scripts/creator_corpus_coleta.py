...
def contratos():
    ...
    for rotulo, actor in ATORES.items():
        ...
        try:
            ...
            build = (d.get('taggedBuilds') or {}).get('latest') or {}
            schema = build.get('inputSchema')
            if isinstance(schema, str):
                schema = json.loads(schema)
            props = (schema or {}).get('properties') or {}
            registro['INPUT_FIELDS'] = sorted(props)
            registro['REQUIRED'] = (schema or {}).get('required') or []
        except Exception as e:                               # noqa: BLE001
            registro.update(STATE='UNREACHABLE',
                            ERROR='%s: %s' % (type(e).__name__, str(e)[:160]))
        fora.append(registro)
        print('%-20s %-34s %s' % (rotulo, actor, registro['STATE']))
        if registro.get('INPUT_FIELDS'):
            print('   campos:', ', '.join(registro['INPUT_FIELDS'][:24]))
