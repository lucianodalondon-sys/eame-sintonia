#!/usr/bin/env python3
"""Roda o parser nos 163 rotulos e escreve o conjunto CANDIDATO de pares.

CANDIDATO, e nao substituto. A regra de publicacao da missao exige precisao E recall
altos e explicitos. A precisao amostrada ficou alta, mas o recall contra o gabarito
ficou em 0,63: ha familias de rotulo que este parser ainda nao le. Trocar o conjunto
antigo por este seria publicar um resultado "maior" sem ser melhor em tudo — que e
...
def main(pdf_dir, saida):
    ...
    for i, p in enumerate(P, 1):
        ...
        if not os.path.exists(pdf):
            sem.append({'REGISTRATION_ID': rid, 'WHY': 'PDF_AUSENTE'})
            continue
    ...
    out = {
        'DATASET': 'IT-ROTULOS-PARES-V2-CANDIDATO',
        ...
        'ESTADO': 'CANDIDATO — NAO substitui productRelationships.json',
        'POR_QUE_CANDIDATO': 'a regra de publicacao exige precisao E recall altos. A '
                             'precisao amostrada ficou alta (23 de 24 pares conferidos a '
                             'mao), mas o recall contra o gabarito manual ficou em 0,63. '
                             'Publicar como substituto seria trocar um conjunto incompleto '
                             'conhecido por outro incompleto ainda nao mapeado.',
