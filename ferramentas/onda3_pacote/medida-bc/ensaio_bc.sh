...
# ── BC: bloco A da LEGACY-99 (so MOSTRAR, ferramenta trazida para a copia, sem juntar codigo) ──
git -C $REPO show origin/legacy-99-v2:curadoria/importar_do_coletor.py > curadoria/importar_do_coletor.py
py -B curadoria/importar_do_coletor.py > $OUT/BC-A-importar-mostrar.txt 2>&1
tail -5 $OUT/BC-A-importar-mostrar.txt | sed 's/^/bloco A: /'
rm -f curadoria/importar_do_coletor.py
