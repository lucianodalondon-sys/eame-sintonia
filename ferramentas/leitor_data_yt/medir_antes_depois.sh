#!/bin/bash
# LEITOR-DATA-YOUTUBE · antes (codigo do vivo) x depois (este ramo), SO LEITURA, sem rede e sem banco.
...
for lado in antes depois; do
  ...
  mkdir -p $W/scripts/leitor_medida && cp $REPO/ferramentas/leitor_data_yt/prever_acervo.py $W/scripts/leitor_medida/prever_acervo.py
