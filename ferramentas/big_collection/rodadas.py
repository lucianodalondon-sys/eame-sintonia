...
"""O DISPARADOR DE RODADAS — a onda que nao cabe no teto, corrida em rodadas, UMA A UMA.
    ...
    py ferramentas/big_collection/rodadas.py --correr --sha256=<da coorte congelada> --base=<pasta>
          [--historico=...] [--rodada=N] [--teto-dia=N] [--livros-do-dia=<pasta das ondas>]
...
com as guardas que a mao esquecia:
  ...
  4. --teto-dia=N (opcional): soma os `TETO-ONDA.json` de HOJE em --livros-do-dia (por omissao, a
...
def correr_rodadas(base: Path, sha: str, plano: dict, *, onda, portao, relatorio, ledger: Path,
                   ...
                   livros_do_dia: Path | None = None, hoje: date | None = None) -> dict:
    ...
    estado.update(BASE=str(base), COORTE_SHA256=sha, TETO_DIA=teto_dia)
    ...
    for r in alvo:
        ...
        reg = {"RODADA": n, "PASTA": str(pasta), "INICIO": agora(), "ESTADO": "A_CORRER"}
        ...
        correm, adiadas = filtrar_pelo_dia(r["FONTES"], gasto_do_dia(livros_do_dia or base.parent, hoje)
...
def main(argv=None) -> int:
    ...
    if "--so-plano" in argv:
        ...
        for r in p["RODADAS"]:
            print("RODADA %02d  %2d fontes  %3d pedidos  max/dominio %d" % (
