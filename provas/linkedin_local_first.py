...
# ⚠️ DUAS RECUSAS SERVEM, E A LEI DISSO E DA PROPRIA LINKEDIN-OP-01.
# A sentinela pedia `SEM_ROTA`. Mas a mesma missao que a escreveu poe
# `linkedin.direct_post` em `BLOCKED`, e um estado que nao promete resultado e
# recusado pelo portao epistemologico ANTES de alguem perguntar pela rota.
# Medido na linha de origem desta prova, e nao so aqui: ela devolve
# `CAPABILITY_STATE_PROMISES_NOTHING` tambem la.
#
#     A RECUSA QUE CHEGA PRIMEIRO E A QUE SE LE.
#
# As duas sao a politica a falar; o que nao pode e passar.
diz(ck2['STATE'] in (ex.SEM_ROTA, ex.SEM_PROMESSA) and not ck2['CAN'],
    'e o post direto continua sem correr, como a politica manda', ck2['STATE'])
