...
def caso_5_asr():
    """A FALA transcrita PELA CASA — e não pelo provedor.

    `TRANSCRIPT` e `ASR` são espécies diferentes no contrato, e o caso 3 só
    cobria a primeira. A distinção não é decorativa: uma transcrição que
    chegou pronta e uma que a casa produziu têm autoridades diferentes, e a
    `kind_basis` diz qual é qual — `DECLARED_BY_PROVIDER` contra
    `PRODUCED_BY_LOCAL_ASR`.

        QUEM PRODUZIU O TEXTO FAZ PARTE DO QUE O TEXTO E.
    """
    unidade = pv.unidade_de_texto(
        texto='benvenuti al diciottesimo numero del periodico',
        kind=pv.ASR, kind_basis=pv.PRODUCED_BY_LOCAL_ASR,
        relation=pv.ORIGINAL, language='it', unit_id='TU-ASR-1',
        source_artifact='https://www.youtube.com/watch?v=GHI',
        derivation_method=pv.ASR_DA_CASA, tool='whisper', model='large-v3')
    return env.envelope(
        platform='YOUTUBE', native_id='GHI',
        url='https://www.youtube.com/watch?v=GHI', content_type='VIDEO',
        route='youtube-data-api-v3:videos.list',
        executor='youtube_oficial.video', run_id=RUN_ID, country_scope='IT',
        source_account='UCxyz', published_at='2026-09-05T10:00:00Z',
        language='it', text_units=[unidade])
...
CASOS = (
    ...
    ('CASE_4_UNKNOWN', caso_4_desconhecido, pv.TEXTO_DESCONHECIDO,
     pv.TEXTO_DESCONHECIDO, pv.TEXTO_DESCONHECIDO),
    ('CASE_5_ASR', caso_5_asr, pv.ASR, pv.ORIGINAL, 'it'),
)
...
def main():
    ...
    # TRES EIXOS, TRES CONTAS. `ESPECIE_PRESERVADA` comparava a terna inteira
    # e reportava tudo sob um numero so — e um numero so nao diz QUAL eixo
    # se perdeu. Espécie, relação e língua partem-se por motivos diferentes e
    # consertam-se em sitios diferentes.
    #
    #     UM CONTADOR PARA TRES PERGUNTAS RESPONDE A UMA.
    perdidas, falhas = [], 0
    perda = {'KIND': 0, 'RELATION': 0, 'LANGUAGE': 0}
    for nome, fabrica, kind_esperado, rel_esperada, lingua_esperada in CASOS:
        ...
        print('   %-56s %s  · esperado=%s obtido=%s'
              % ('ESPECIE_PRESERVADA', 'PASSA' if ok else 'PARA',
                 esperado, obtido))
        for eixo, i in (('KIND', 0), ('RELATION', 1), ('LANGUAGE', 2)):
    ...
    print('TEXT_KIND_LOSS = %d' % perda['KIND'])
    print('TEXT_RELATION_LOSS = %d' % perda['RELATION'])
    print('LANGUAGE_LOSS = %d' % perda['LANGUAGE'])
    print('CASOS_PERDIDOS = %d' % falhas)
