...
que rebaixou.
    ...
    python3 medidas/cicatrizes_brasil.py            # mede e imprime
    python3 medidas/cicatrizes_brasil.py --build    # grava o artefato
"""
import json
import os
import re
...
def acervo():
    return {
        ...
        'scripts': ''.join(texto_de(g) for g in
                   ('fontes', 'coleta', 'regras', 'ferramentas', 'guarda',
                    'leis', 'motor', 'provas', 'pacote', 'portoes',
                    'superficie')),
        'docs': texto_de('docs'),
...
CICATRIZES = [
 ...
 {'ID': 'BR-19', 'FAMILIA': 'RESILIENCIA',
  ...
  'OWNER': 'checkpoint_coleta (016) com os nove campos mínimos, e pode_gastar(), '
           'chamada por coleta/coleta_checkpoint.py ANTES da primeira chamada paga. '
 ...
 {'ID': 'BR-21', 'FAMILIA': 'RESILIENCIA',
  ...
  'EAME_APPLICABLE': 'YES', 'EAME_STATUS': 'PROVED',
  'OWNER': 'ferramentas/apify_pool.py, PORTADO da Itália sem segunda implementação, sob '
           'coleta/coleta_checkpoint.py. Token esgota -> parcial preservado -> '
