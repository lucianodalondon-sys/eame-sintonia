...
# ── UMA SEGUNDA `proxima_tentativa` VIVIA AQUI, E FOI RETIRADA ───────────
# O merge trouxe-a do SCRAP sem conflito — duas definicoes do mesmo nome nao
# colidem para o git — e em Python a SEGUNDA ganha. Ela era keyword-only
# (`banco, *, run_id, etapa`), e TRES chamadores desta casa chamam-na por
# posicao: `ingresso`, `derivacao_forward` e `rota_forward_documento`. O
# quarto, `coleta_checkpoint`, chama por nome.
#
#     A ASSINATURA QUE FICA E A QUE SERVE OS QUATRO, E NAO A MAIS RECENTE.
#
# A de cima aceita as duas formas e tem a mesma consulta. Nada se perdeu.
