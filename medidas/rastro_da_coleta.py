...
ESTADOS = (PASS, REJECTED, UNKNOWN, ERROR, NOT_RUN, SKIPPED, NOT_APPLICABLE)
...
def registrar(banco, *, run_id, etapa, estado, edge_from=None, tentativa=0,
              ...
              passed=0, rejected=0, unknown=0, error=0, skipped=0,
    ...
    `diagnostic_code` nao precisa vir pronto: quando o estado e ERROR e ninguem
    ...
    if estado == ERROR and not diagnostic_code:
    ...
    colunas = {
        ...
        'passed': passed, 'rejected': rejected, 'unknown_count': unknown,
        'error_count': error, 'skipped': skipped, 'estado': estado,
...
def passagens(banco, *, run_id):
    linhas = banco.executa(
        ...
        " passed, rejected, unknown_count, error_count, skipped,"
    ...
    campos = ('ETAPA', 'EDGE_FROM', 'ESTADO', 'INPUT_GRAIN', 'INPUT_COUNT',
              'OUTPUT_GRAIN', 'OUTPUT_COUNT', 'PASSED', 'REJECTED', 'UNKNOWN',
              'ERROR', 'SKIPPED', 'ACCOUNTED', 'UNACCOUNTED', 'DIAGNOSTIC_CODE',
              'LAST_GOOD_ARTIFACT', 'DURACAO_MS', 'CUSTO_USD', 'TENTATIVA')
    saida = []
    for l in linhas:
        d = dict(zip(campos, l[:len(campos)]))
        for k in ('INPUT_COUNT', 'OUTPUT_COUNT', 'PASSED', 'REJECTED', 'UNKNOWN',
                  'ERROR', 'SKIPPED', 'ACCOUNTED', 'UNACCOUNTED', 'DURACAO_MS',
                  'TENTATIVA'):
            d[k] = int(d[k])
...
def onde_retomar(passagens_da_corrida):
    ...
    erros = [p for p in passagens_da_corrida if p['ESTADO'] == ERROR]
    if not erros:
        return None
