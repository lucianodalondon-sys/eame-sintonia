...
--
-- Gerado por guarda/catalogo_importar.py --sql a partir de
...
def gerar_sql(L):
    ...
    # ── ONDE MORA O ID DO MAPA, HOJE ─────────────────────────────────
    # Este importador nasceu resolvendo por `crop.mapa_id_cultivo` e
    # `issue.mapa_id_plaga`. Essas colunas NÃO EXISTEM MAIS: a migration
    # 009 as removeu de propósito, e o comentário dela diz por quê —
    #
    #   "nome_es e mapa_id_cultivo eram seed espanhol dentro da entidade
    #    canônica. OLIVE é OLIVE nos três países, e o id do MAPA é um
    #    identificador de UM sistema nacional."
    #
    # Recriá-las para fazer este import rodar seria desfazer a isolação de
    # país por conveniência de importação — e criar um segundo dono da
    # mesma pergunta, porque o dono existe: `crop_local` / `issue_local`,
    # com (pais, source_system, external_id).
    #
    # A lei NO_FUZZY_SILENT_MATCH não muda: sem casar, fica NULL e o rótulo
    # publicado permanece. O que muda é POR ONDE se procura o casamento —
    # e agora a busca é explicitamente espanhola, em vez de assumir que
    # todo id numérico numa entidade canônica é do MAPA.
    def cropsel(mapa_id):
        return ('null' if not mapa_id else
                "(select crop_id from public.crop_local where pais='ES' and "
                "source_system='MAPA_ROPF' and external_id=%s)" % q(str(mapa_id)))

    def issuesel(mapa_id):
        return ('null' if not mapa_id else
                "(select issue_id from public.issue_local where pais='ES' and "
                "source_system='MAPA_ROPF' and external_id=%s)" % q(str(mapa_id)))
    ...
    for j in L['JANELA']:
        out.append("insert into public.catalogo_produto_janela_aplicacao (produto_id,"
                   ...
                   # A identidade da janela e a LINHA DO DOCUMENTO que a
                   # sustenta, por produto. Sem isto o import nao era
                   # idempotente: 3 janelas viravam 6 na segunda execucao.
                   " on conflict (produto_id, ancora_secao, ancora_texto)"
                   " do nothing;\n"
    ...
    for c in L['CROSSWALK']:
        ...
        out.append("insert into public.catalogo_registro_crosswalk (captura_id, produto_id,"
                   ...
                   " values (%s,%s,%s,%s,%s,%s)"
                   # A identidade e o PAR que a captura afirma. Sem isto o
                   # crosswalk ia de 108 para 216 na segunda execucao — e os
                   # 44 LOCAL_REGISTERED viravam 88.
                   " on conflict (captura_id, produto_id, registration_id_texto)"
                   " do nothing;\n"
