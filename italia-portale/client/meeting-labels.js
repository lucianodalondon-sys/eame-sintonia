/* SINTONIA ITALIA · IL DIZIONARIO DELLA SUPERFICIE CANONICA (IT · EN)
   ---------------------------------------------------------------------------
   Il motore parla in CODICI, e i codici sono la ragione per cui due lingue
   possono leggere la stessa intelligence senza tradurre una decisione. Ma un
   codice non e un'etichetta: `RULE_DELEGATED_TO_FARM` su uno schermo italiano
   e un difetto, non un fatto.

       IL MOTORE DECIDE IN CODICI. LO SCHERMO LEGGE IN LINGUA.
       QUESTO FILE E LA FRONTIERA FRA LE DUE COSE, E NON DECIDE NULLA.

   Nessuna voce qui dentro calcola, sceglie o ordina. Ogni voce e la stessa
   cosa detta in italiano e in inglese. Un codice che il motore aggiunge
   domani e ASSENTE — visibile come tale — invece di essere reso a caso: il
   gate IT_LABELS_COMPLETE / EN_LABELS_COMPLETE reprova la mancanza.

   UNKNOWN NON PUO SPARIRE DIETRO UNA COPY BELLA. Dove il motore non sa,
   queste frasi dicono che non si sa, e dicono cosa manca perche si sappia. */
window.MEETING_LABELS = (function () {
  'use strict';
  var F = {};
  F["STATUS"] = {
  ...
  /* Un codice non tradotto non diventa una parola inventata: esce come stringa
     vuota e il gate lo trova. Meglio un vuoto misurabile di un token dipinto. */
  function t(family, code, lang) {
    if (code === null || code === undefined || code === '') return '';
    var f = F[family]; if (!f) return '';
    var e = f[String(code)]; if (!e) return '';
    return lang === 'en' ? e[1] : e[0];
  }
  function has(family, code) {
    var f = F[family]; return !!(f && f[String(code)]);
  }
  function list(family, codes, lang) {
    return (codes || []).map(function (c) { return t(family, c, lang); })
      .filter(function (s) { return !!s; });
  }
  return { families: F, t: t, has: has, list: list };
})();
