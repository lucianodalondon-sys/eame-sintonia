-- ═══════════════════════════════════════════════════════════════════════
-- EAME SINTONIA — MIGRATION 014
-- CATÁLOGO PÚBLICO DO FABRICANTE — a terceira afirmação, com casa própria.
--
-- ⚠️ RENUMERADA. Nasceu como 010 na branch claude/adama-es-local-browser,
-- quando o core tinha 009 migrations. O core seguiu em paralelo e ocupou
-- 010, 011 e 012 com o calendário agronômico; o slot 014 foi deixado VAGO
-- de propósito, desde a 015, esperando por este arquivo.
--
-- A renumeração é o ÚNICO conteúdo alterado. O corpo veio verbatim de
-- 0ed66f2:supabase/migrations/010_catalogo_publico_fabricante.sql.
--
-- HANDOFF_SCHEMA_VALID_THEN != SCHEMA_VALID_NOW, e por isso o contrato foi
-- reconferido contra o schema de hoje antes de entrar, e não copiado:
--
--   · 15 tabelas, todas com prefixo catalogo_, ZERO colisão de nome com os
--     objetos que o core criou entre a 010 e a 018;
--   · os cinco donos que ela referencia — collection_run, raw_asset,
--     registro_regulatorio, crop, issue — existem e não mudaram de forma
--     nas migrations 013 a 018;
--   · ela NÃO toca conteudo.fact_geografia_id, aposentada pela 018, nem
--     recria captura, proveniência, localização, tempo ou identidade.
--     O catálogo REFERENCIA os donos; não os refaz.
--
-- ORDEM: depois da 013 (CAPTURE != REGISTRATION, que dá o dono da captura
-- regulatória que o crosswalk aponta) e antes da 015. A 008 continua sendo
-- a última de todas.
--
...
--
-- NÃO EXECUTADA em Supabase quando foi escrita. Aplicada pela primeira vez
-- na missão de importação controlada, pelo caminho canônico de produção.
-- ═══════════════════════════════════════════════════════════════════════
...
,
  ...
  -- ── CHAVE NATURAL, acrescentada na integração de 2026-08-30 ─────────
  -- A tabela nasceu sem nenhuma, e o teste de idempotência da missão de
  -- importação pegou: rodar o mesmo import duas vezes levava as janelas de
  -- 3 para 6. Um INSERT sem conflito declarado não é idempotente — ele só
  -- ainda não tinha rodado duas vezes.
  --
  -- A identidade de uma janela é a LINHA DO DOCUMENTO que a sustenta, por
  -- produto: duas leituras do mesmo trecho são a mesma janela. Por isso a
  -- âncora entra na chave, e não os valores de BBCH — dois trechos
  -- diferentes podem dizer o mesmo BBCH e continuam sendo duas afirmações.
  UNIQUE NULLS NOT DISTINCT (produto_id, ancora_secao, ancora_texto)
);
...
,
  ...
  -- ── CHAVE NATURAL, acrescentada na integração de 2026-08-30 ─────────
  -- Mesma história da janela, e mais cara: sem chave, o segundo import
  -- levava o crosswalk de 108 para 216 — e com ele os 44 LOCAL_REGISTERED
  -- viravam 88 e os 12 não-provados viravam 24. Números de gate dobrando
  -- porque um arquivo rodou duas vezes.
  --
  -- A identidade é o PAR que a captura afirma: um lado de catálogo, um lado
  -- de registro. NULLS NOT DISTINCT é obrigatório aqui — ROPF_ONLY tem
  -- produto_id nulo e ADAMA_SITE_ONLY tem registration_id_texto nulo, e sem
  -- isso os dois grupos escapariam da trava justamente por serem os que têm
  -- um lado só.
  --
  -- `estado` fica FORA da chave de propósito: a mesma dupla mudando de
  -- MATCHED_WITH_EVIDENCE para MATCHED_EXACT não é uma linha nova, é a
  -- mesma afirmação com outra força — e duas linhas dela seria o CUPROXI
  -- fundido pela porta dos fundos.
  UNIQUE NULLS NOT DISTINCT (captura_id, produto_id, registration_id_texto)
);

-- ── A TRAVA QUE FALTAVA, achada pelo red team da integração ──────────
-- O red team tentou virar um MATCHED_EXACT em ADAMA_SITE_ONLY e o banco
-- ACEITOU: nada dizia que "presente só no site da ADAMA" significa que
-- NENHUM registro foi encontrado. Uma linha podia afirmar não-provado e
-- carregar o número do registro ao mesmo tempo.
--
-- Isso quebra a lei nos dois sentidos: um vínculo provado podia ser
-- rebaixado sem perder a prova (e voltar depois), e um não-provado podia
-- exibir um número de registro que a própria linha diz não ter achado.
--
--   LOCAL_PRESENT_BUT_REGISTRATION_NOT_PROVED != NOT_REGISTERED
--   e também != REGISTRADO-COM-O-NÚMERO-AQUI-DO-LADO
alter table public.catalogo_registro_crosswalk
  drop constraint if exists site_only_nao_carrega_registro;
alter table public.catalogo_registro_crosswalk
  add constraint site_only_nao_carrega_registro
  check (estado <> 'ADAMA_SITE_ONLY'
         or (registro_id is null and registration_id_texto is null));
