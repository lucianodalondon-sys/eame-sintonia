...
def autor_hash(author_channel_id):
    """Identificador do autor -> `autor_hash` de 64 hex. NUNCA nome, NUNCA @.
    ...
    """
    if not author_channel_id or author_channel_id == 'UNKNOWN':
        return None
    return hmac.new(ip._sal(), str(author_channel_id).strip().encode('utf-8'),
...
def persistir_video(banco, *, canal_id, run_id, content_id, texto_canonico,
    ...
    return {'STATE': 'OK', 'CONTEUDO_ID': conteudo_id, 'CONTENT_ID': content_id,
    ...
    for c in comentarios:
        ...
        sql = (
            ...
            % (_lit(conteudo_id), _lit(run_id), _lit(externo), _lit(pai),
               _lit(autor_hash(bruto.get('AUTHOR_CHANNEL_ID'))), _lit(texto),
