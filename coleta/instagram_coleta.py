...
INSTAGRAM — o orquestrador. Uma fase por comando, um teto por fase, um manifesto por fase.
    ...
    py coleta/instagram_coleta.py contratos      # GRÁTIS · lê o schema de cada ator
    py coleta/instagram_coleta.py plano          # GRÁTIS · em que plano está cada chave
    py coleta/instagram_coleta.py janela         # GRÁTIS · a rota do navegador
    py coleta/instagram_coleta.py bio            # PAGA   · teste de fumaça de ~1 centavo
    py coleta/instagram_coleta.py posts          # PAGA   · um run por conta
    py coleta/instagram_coleta.py reels          # PAGA   · um run por conta
    py coleta/instagram_coleta.py comentarios    # PAGA   · atrás do portão de dado pessoal
    py coleta/instagram_coleta.py liquidar       # GRÁTIS · o custo REAL, depois
    py coleta/instagram_coleta.py semaforo       # GRÁTIS · a grade de cobertura
...
def plano():
    ...
    if not chaves:
        print('POOL_EMPTY — sem chave não dá para ler o plano. A leitura do CONTRATO, '
              'essa sim, roda sem chave: `py coleta/instagram_coleta.py contratos`.')
        return 1
    linhas = []
...
def _rodar(fase, entrada, *, run_id, conta, evidencia, autorizacao=None):
    ...
    ⚠️ E O LOOP DE ROTAÇÃO É EXACTAMENTE ONDE UMA AUTORIZAÇÃO SÓ SE GASTARIA
    UMA VEZ E PAGARIA N. Ele percorre o pool inteiro e chama a porta paga a
    cada posição: com cinco chaves no cofre, uma autorização de uma execução
    compraria cinco. Por isso `coletor.executar` CONSOME a autorização a cada
    passagem, e a sexta recusa com `AUTORIZACAO_ESGOTADA`.
...
def _gravar_fase(fase, cs, itens, mans, parciais, dias):
    ...
    print('  custo: NÃO LIQUIDADO — rode `py coleta/instagram_coleta.py liquidar`')
    return 0
...
def comentarios(por_post=20):
    ...
    caminho = _gravar('COMENTARIOS.json', {
        ...
        'PURGE_HOW': 'py coleta/instagram_pessoal.py expurgar --confirmar',
