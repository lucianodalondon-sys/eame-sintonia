...
INSTAGRAM PELA JANELA — tudo o que a rota pública entrega antes de gastar um centavo.
    ...
    py coleta/instagram_janela.py perfis     # bio, seguidores, denominador, a grade
    py coleta/instagram_janela.py objetos    # data, legenda inteira, curtidas, vídeo
    py coleta/instagram_janela.py tudo       # os dois, na ordem
...
def objetos(limite_por_conta=None):
    ...
    if not fonte:
        print('SEM PERFIS — rode `py coleta/instagram_janela.py perfis` antes')
        return 1
    cdp.subir(PORTA, perfil=PERFIL)
