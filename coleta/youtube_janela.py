...
YOUTUBE PELA JANELA — tudo o que a rota pública entrega antes de gastar um centavo.
    ...
    py coleta/youtube_janela.py canais      # nome, inscritos, descrição, a grade
    py coleta/youtube_janela.py objetos     # título, duração, views, data, videoId
    py coleta/youtube_janela.py legendas    # a LEGENDA, que é a transcrição de graça
    py coleta/youtube_janela.py tudo        # os três, na ordem
...
def fase_objetos(limite=None):
    ...
    if not canais:
        print('sem CANAIS.json — rode `py coleta/youtube_janela.py canais` antes')
        return 1
...
def fase_legendas(limite=None):
    ...
    if not objetos:
        print('sem OBJETOS.json — rode `py coleta/youtube_janela.py objetos` antes')
        return 1
    alvos = objetos['ITEMS']
    if limite:
