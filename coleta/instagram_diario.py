...
O MONITOR DIÁRIO — o que o concorrente publicou hoje, e o que disso é notícia.
    ...
    py coleta/instagram_diario.py fila       # GRÁTIS: quem vence hoje, e por quê
    py coleta/instagram_diario.py rodar      # a passada do dia
    py coleta/instagram_diario.py noticia    # GRÁTIS: o que MUDOU desde ontem
...
def noticia():
    ...
    for n in sorted(os.listdir(SAIDA), reverse=True):
        if n.startswith('PASSADA-'):
            ...
            break
    if not passada:
        print('nenhuma passada ainda. Rode `py coleta/instagram_diario.py rodar`.')
        return 1
