...
> autorização como **universos separados** (junção por número de registro).
    ...
    VEREDITO DESTA REVISÃO: NOT_READY — dois defeitos medidos, ambos no
    vínculo da JANELA e na leitura de UMA oração corrida. Nenhum deles inventa
    oportunidade; os dois destroem oportunidade verdadeira.

    ➜ OS DOIS FORAM CORRIGIDOS DEPOIS DESTA REVISÃO. O fechamento, com o
      antes-e-depois medido caso a caso e um TERCEIRO defeito encontrado na
      revisão humana dos SALES_READY, está na **§9 · FECHAMENTO**, no fim
      deste documento. O veredito vigente é o da §9.
...
# 9 · FECHAMENTO — os dois defeitos corrigidos, e um terceiro achado

> Missão seguinte a esta revisão. Corrigidos **na branch de produção**
> `claude/site-v21-ingest-recovery`, com a branch da revisão
> (`claude/opportunity-commercial-priority-v1`) **intacta**: os arquivos foram
> **portados**, não substituídos, e nada foi empurrado para lá.

    VEREDITO VIGENTE: READY_TO_MERGE

## 9.1 · `JANELA` — o vínculo passou a ser (cultura, alvo) com contenção de região

`win_crop = _ix(cs['CROP-WINDOWS'], 'CROP_IDS')` deixou de existir. No lugar,
`win_par[(cultura, alvo)]` mais `janelas_do_caso(crop, alvo, geo)`, que só
devolve a janela quando ela **declara o mesmo alvo** e quando a **geografia dela
contém a do caso**. A contenção é num sentido só: uma janela provincial de
Modena não fala pela Itália, e aceitá-la num caso nacional seria a mesma
promoção de geografia, ao contrário.

Uma janela **sem alvo declarado** (`IT-WIN-006`) não entra em caso com alvo —
ela não fala de alvo nenhum.

**MEDIDO, antes → depois, sobre as mesmas 43 oportunidades:**

| | antes | depois |
|---|---:|---:|
| casos citando janela de OUTRO alvo/região | 12 | **0** |
| casos exibindo `2027-05-31` (data de ato) como janela | 16 | **0** |
| casos com família externa `CROP_WINDOW` inflada | 12 | **0** |
| `OPPORTUNITY_CONFIRMED` | 21 | **33** |
| portões `A_GEOGRAFIA` / `F_PROCEDENCIA` abertos por janela alheia | 12 | **0** |

Os 12 portões fecharam porque a causa deles era o vínculo, não a evidência — é
o que esta mesma revisão previa em §3. **Nenhuma oportunidade foi criada:** o
conjunto continua com 43, com as mesmas identidades de arquétipo × cultura ×
alvo × região. O que mudou foi o que cada uma pode citar.

Depois da correção, **nenhum** dos 43 casos cita janela nenhuma: as sete de
`CROP-WINDOWS` são seis de videira × *Scaphoideus* (Veneto, Lombardia, Piemonte,
Trentino, Emilia-Romagna) mais uma de oliveira × mosca, e nenhum caso `O1` cai
sobre esse par nessas regiões. É o resultado honesto — a janela existe, e não é
a deste caso.

## 9.2 · `NEED_DIRECTION` — a direção passou a ser por ALVO, não por oração

O cortador de orações quebra em `.` e `;`, e `IT-PHEN-041` (Firenze) publica o
mesmo texto de `IT-PHEN-040` (Siena) **com vírgulas** onde Siena usa ponto e
vírgula. A oração inteira chegava com cinco assuntos dentro, o primeiro padrão
que casava era `suspensao`, e a direção ia para os três alvos com `ISSUE_ID`.

A correção não adivinha e não afrouxa:

1. a oração que nomeia **um** alvo continua exatamente como estava;
2. a que nomeia **mais de um** é subdividida **na vírgula** — a mesma
   enumeração que Siena escreveu com ponto e vírgula — e cada pedaço que isola
   **um** alvo responde por ele, com a direção que esse pedaço declara;
3. o alvo que nenhum pedaço isola fica `NEUTRAL_MENTION`, e a ambiguidade é
   **escrita** no pino (`NEED_AMBIGUITY`) em vez de virar palpite silencioso.

`NEUTRAL_MENTION` não abre porta comercial nenhuma — cai em `NECESSIDADE_MORNA`
e o caso sai `TO_VALIDATE`, o mesmo destino de quem tem a porta fechada. O
passo 3 nunca converte uma proibição em venda.

**MEDIDO:** dos 49 pinos, **exatamente dois** mudaram, os dois em `IT-PHEN-041`:

| pino | antes | depois | o trecho que agora viaja com ele |
|---|---|---|---|
| videira × botrite · Toscana | `ACTION_SUSPENDED` | `NEUTRAL_MENTION` | «janela de maior suscetibilidade a botrite» |
| videira × *Scaphoideus* · Toscana | `ACTION_SUSPENDED` | `WINDOW_CONCLUDED` | «fim da defesa de Scaphoideus titanus.» |

Nenhum par apareceu, nenhum desapareceu. E `videira × botrite · Toscana`
deixou de ser suprimido pela leitura falsa da região vizinha: agora responde
por ele o pino de Siena, «**possivel intervir** com antibotriticos…».

**Um buraco de vocabulário veio à tona junto** e foi fechado: o padrão de
suspensão exigia as palavras `defesa` ou `tratamento` («suspensao da defesa
antiperonosporica») e não via «suspensao **de oidio**», que nomeia o ALVO no
lugar da defesa. Enquanto a oração corrida era lida inteira, o buraco ficava
escondido — a suspensão da peronospora cobria os vizinhos.

    SUSPENDER O OIDIO É SUSPENDER O TRATAMENTO DO OIDIO.

## 9.3 · O terceiro defeito, achado na revisão humana dos SALES_READY

Os cinco `SALES_READY` foram lidos ficha a ficha contra as oito perguntas da §1.
Três deles têm frase de fonte que **não manda apenas tratar**: manda tratar
**com substâncias nomeadas**, e a substância do produto ADAMA do caso não está
entre elas.

| caso | a fonte prescreve | o produto do catálogo é |
|---|---|---|
| videira × botrite · Emilia-Romagna | «Fenhexamid (max 2) ou alternativas biologicas» | BANJO = **FLUAZINAM** |
| videira × tignoletta · Emilia-Romagna | «Bacillus t., Emamectina (max 2) ou Spinosad (max 3)» | Lamdex® Extra = **LAMBDA-CYHALOTHRIN** |
| videira × botrite · Toscana | «antibotriticos microbiologicos, bicarbonato de potassio ou terpenos» | BANJO = **FLUAZINAM** |

O rótulo ministerial cobre o par nos três, e por isso a **coluna interna não se
mexe**: a venda continua defensável. O que não sobrevive é o material que sai de
casa. O `(max 2)` é o teto da **defesa integrada regional**, e a frase do
serviço ao lado do nosso produto seria lida como «o serviço recomenda este
produto».
...
Nasce daí o **sexto bloqueio externo**, `SOURCE_PRESCRIBES_OTHER_MEANS`. A regra
**não lê química**: lê a frase. O boletim prescreve o meio numa oração
«…intervir/tratar/defesa **com** ‹meio›», e é só essa oração que se examina.
Onde não há oração de meio, nada é bloqueado — e os dois casos que esta revisão
verificou à mão continuam `YES`:

* carpocapsa · Veneto — «terceiro voo terminado **com danos em aumento**»: o
  «com» é adjunto de um relato, não prescrição de meio;
* piralide · Friuli — «tratamento insecticida justificado quando se observarem
  posturas superiores a 3 por 100 plantas»: limiar, sem meio nomeado.

A prova viaja com o caso: `SOURCE_PRESCRIBED_MEANS` (a oração de meio, íntegra —
o ponto de «Bacillus **t.**» é abreviatura e não corta a prova ao meio) e
`CASE_ACTIVE_INGREDIENTS` (a substância do nosso produto), lado a lado, para
quem quiser discordar sem reabrir o motor.
...
OPPORTUNITY_CONFIRMED             = 33   (era 21)
SALES_READY                       = 5    (era 4)
EXTERNAL_MATERIAL_READY = YES     = 2    (carpocapsa · Veneto · piralide · Friuli)
EXTERNAL VALIDATION_REQUIRED      = 3    (os três de 9.3)
EXTERNAL NO                       = 38

CASOS REGIONAIS   = PASS
NEED_DIRECTION    = PASS   (corrigido · 9.2)
CROP_TARGET       = PASS
CATÁLOGO          = PASS
JANELA            = PASS   (corrigido · 9.1)
MATERIAL EXTERNO  = PASS   (sexto bloqueio · 9.3)

PROVAS DA CAMADA COMERCIAL = 58 de 58 verdes  (eram 42; +10 em T19/T20, +6 em T21)
SUÍTE COMPLETA = 721 testes · 6 falhas · 1 erro · 16 pulados
                 — as falhas são as da §7, todas anteriores a esta linha de
                   missões, nenhuma tocando a camada comercial.
DURUM_WHEAT = COLLECTION_REQUIRED   (inalterado · §6)
```

**O `SALES_READY` que nasceu da correção** é `videira × botrite · Toscana`, e
ele é exatamente o que a §4 previa: a oração corrida de Firenze o suprimia. Os
outros quatro já existiam. Nenhum caso foi promovido por afrouxamento de
portão — todos os portões continuam onde estavam, e três dos cinco continuam
proibidos de sair de casa.

## 9.5 · O que ficou de fora, e por quê

* **`DURUM WHEAT`** continua `HONEST_UNKNOWN`. A coleta das 14 etiquetas
  descrita na §6 não foi feita: é coleta externa, e esta missão não coletou.
* **As 6 falhas e 1 erro da suíte** continuam abertos e continuam classificados
