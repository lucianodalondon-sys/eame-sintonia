...
| Dedupe é constraint, não auditoria posterior | `documentos` → `unique(fonte_id, hash_conteudo)` | o banco recusa a segunda inserção | portão `PIPELINE_DEDUPE` em `medidas/portao.py` | **ADAPT** | manter o portão *e* ganhar o unique — um denuncia, o outro impede |
