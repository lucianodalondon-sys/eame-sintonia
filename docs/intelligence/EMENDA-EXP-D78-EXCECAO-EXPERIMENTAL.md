...
1. esta emenda (lado Intelligence);
2. uma linha no contrato da trava (lado Collection, pelo dono dela) acrescentando
   a `O_QUE_A_TRAVA_NAO_IMPEDE`: *«execução EXPERIMENTAL só-leitura da
   Intelligence, nos termos da EMENDA EXP-D78»* — **sem** mudar
   `COLLECTION_FOUNDATION_CLOSED`, que continua `NAO`.
...
1. **Só-leitura imposto no banco**: `default_transaction_read_only=on` em cada
   ligação, **conferido no servidor** (`show default_transaction_read_only = on`)
   antes de ler.
   ⚠️ Medido: o dono da Sala (`_ambiente_psql()`) **sobrescreve** `PGOPTIONS`;
   o só-leitura de quem chama não chega ao banco. A execução impõe-no em processo
   e prova-o no servidor; o conserto no dono é da Collection (DEFEITO D9).
2. **Fotografia antes/depois** das contagens da Sala, revisões, raw, derivados e
   corridas: tem de ser **igual**, senão o resultado é `INVALID`.
...
experimental. Os consertos (D1–D4, D6–D8 do lado Intelligence) são pequenos e
ficam para missão autorizada; D9–D10 são pedidos à Collection.
...
| «só leitura» só declarado | 32-A.2(1): conferido no servidor; D9 prova que o pedido do chamador não bastava |
