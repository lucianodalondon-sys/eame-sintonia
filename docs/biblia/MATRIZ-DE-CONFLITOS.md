...
| **FONTE A** | `data/samples/RUN-MANIFEST.json` — 13 corridas, 22 campos, lido por 5 réguas (`padrao_da_coleta`, `portao`, `proveniencia`, `sensor_coleta`, `contrato_ator`) e assinado por `orquestrador/orquestrador.py` |
...
| **FONTE A** | `orquestrador/orquestrador.py:234-239` — grava `COST_USD = 0` para ensaio seco e rota gratuita, e `"NAO SEI"` no resto; o comentário explica que escrever `NÃO SEI` numa corrida que não abriu ligação seria *inventar dívida* |
...
| **FONTE A** | `orquestrador/orquestrador.py:240-241` — `ITEM_COUNT_RAW` e `ITEM_COUNT_NORMALIZED` nascem `"NAO SEI"` e só são preenchidos **se a colheita for encontrada e passar pela admissão** |
