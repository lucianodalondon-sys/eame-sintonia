...
# C4C · A INVESTIGAÇÃO DO CORPUS — **vive noutra branch**

A missão C4C recuperou os oito artefatos RAW legados, provou-os por hash e
mediu a metade CPU do banco sobre corpus real. Ela também provou que o corpus e
a placa estão em **máquinas diferentes**, e por isso a metade GPU ficou
`BLOCKED_CORPUS_NOT_ON_GPU_MACHINE`.

**Nada disso foi portado para esta linha, e a decisão é deliberada.**

```
branch   claude/sintonia-scrap-gpu-quality-c4c
head     06b5055d2acefe6e462cf9ac170a2a24eccdf018
```

O que lá ficou, e porquê:

| o que | porque não veio |
|---|---|
| `provas/corpus_recuperar.py` | lê o manifesto dos **oito Reels legados**. É testemunha de uma investigação, não capacidade desta linha |
| `tests/test_c4c_corpus.py` | segue o anterior |
| fase `gpu-bench` | depende de um corpus que não está na máquina da placa — uma fase que hoje não pode passar |
| o benchmark CPU sobre os 4 Reels | resultado de uma corrida, não código operacional |

```
ISTO E CAPACIDADE OPERACIONAL, OU TESTEMUNHA HISTORICA DE UMA INVESTIGACAO?
A SEGUNDA FICA ONDE NASCEU. INCHAR A LINHA OPERACIONAL COM ELA TEM CUSTO
E NAO TEM BENEFICIO.
```

O que **veio** foi o delta durável que a C4B produziu e a C4C exercitou: o trace
verdadeiro do dispositivo, o runtime CUDA process-local, a prova técnica da
placa e os consumidores corrigidos. Esses estão nas secções **B2**, **B3**,
**B4**, **H2** e **G2** acima.
