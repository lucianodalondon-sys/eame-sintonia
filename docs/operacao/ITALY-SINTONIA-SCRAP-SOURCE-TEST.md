...
| script | o que faz |
|---|---|
| `scripts/italy_probe.mjs` | mede a porta de todas as fontes; não preserva nada |
| `scripts/italy_find_docs.mjs` | abre uma página e lista os links que parecem documento real |
| `scripts/italy_preserve.mjs` | baixa e preserva com MIME/BYTES/SHA256; **confere a assinatura dos bytes** |
| `scripts/pdf_peek.mjs` · `scripts/html_text.mjs` | conferem que o arquivo preservado é o documento que se diz |
| `scripts/italy_fill_manifests.mjs` | escreve os campos de julgamento decididos à mão |
| `scripts/italy_contract_test.mjs` | Fase H — os guardas do contrato |
...
Foi escrito um guarda em Node para o contrato novo — `scripts/italy_contract_test.mjs`.
