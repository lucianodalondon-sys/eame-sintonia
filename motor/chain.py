...
def fr_prothioconazole(raw=None, molecule='prothioconazole'):
    ...
    steps.append(('AUTOMATIC', 'coleta/ephy.sh download — resolve o recurso semanal pela '
                               'API do data.gouv e descompacta'))
    if not os.path.exists(prod):
        raise ChainFailure(f'FR: {prod} ausente. Rode coleta/ephy.sh download {raw}. '
...
def raif_download(cultura='Olivar', dest=None, only=None):
    ...
    binário `unzip`, que o repositório já exigia em `coleta/ephy.sh`. Descoberto ao
