...
if [ "${SINTONIA_CADEIA_HISTORICA:-}" != "eu-sei-que-esta-atrasada" ]; then
  ...
  Para inspeccionar a cadeia historica sem a servir a ninguem:
      SINTONIA_CADEIA_HISTORICA=eu-sei-que-esta-atrasada bash motor/v21_cadeia.sh
...
"$PY" motor/v21_ingest.py
"$PY" motor/v21_ingest_b.py
...
"$PY" leis/v21_carimbar_origem.py

echo
echo "── 5 · as fontes, rechaveadas para a chave que o pacote já cita ────────"
"$PY" motor/v21_fontes_rechavear.py
...
"$PY" motor/v21_fontes_faltantes.py
...
"$PY" motor/v21_dominio_da_alegacao.py
...
"$PY" leis/v21_geografia_contrato.py
"$PY" leis/v21_procedencia_contrato.py
