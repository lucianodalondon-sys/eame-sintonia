...
BLOQUEIO_EXTERNO = {
 ...
 'SOURCE_PRESCRIBES_OTHER_MEANS': 'a frase da fonte prescreve MEIOS NOMEADOS e '
                                  'nenhum deles é a substância ativa do produto '
                                  'ADAMA deste caso. O material sairia com a '
                                  'recomendação do serviço ao lado de um produto '
                                  'que ela não nomeia — ver NEED_EXCERPT e '
                                  'SOURCE_PRESCRIBED_MEANS.',
}
...
# ── O SEXTO BLOQUEIO · A FONTE PRESCREVE, E NÃO PRESCREVE O NOSSO ───────────
# Medido na revisão humana dos SALES_READY, depois de corrigidos os dois
# defeitos da V1.1. Três dos cinco casos têm frase de fonte que não manda apenas
# TRATAR: ela manda tratar COM SUBSTÂNCIAS NOMEADAS, e a substância do produto
# ADAMA do caso não está entre elas.
#
#   videira × botrite · Emilia-Romagna «intervir em pre-colheita com Fenhexamid
#                                       (max 2) ou alternativas biologicas»
#                                       produto do catálogo: BANJO = FLUAZINAM
#   videira × tignoletta · Emilia-R.   «intervir com Bacillus t., Emamectina
#                                       (max 2) ou Spinosad (max 3)»
#                                       catálogo: Lamdex® Extra = LAMBDA-CYHALOTHRIN
#   videira × botrite · Toscana        «possivel intervir com antibotriticos
#                                       microbiologicos, bicarbonato de potassio
#                                       ou terpenos»          catálogo: BANJO
#
# O rótulo ministerial cobre o par nos três — a venda interna continua
# defensável e a coluna interna NÃO se mexe. O que não sobrevive é o material
# que sai de casa: o «(max 2)» é o teto da defesa integrada REGIONAL, e a frase
# ao lado do nosso produto seria lida como «o serviço recomenda este produto».
#
...
#
# A regra NÃO lê química: ela lê a frase. O boletim prescreve o meio numa oração
# «...tratar/intervir/defesa COM <meio>», e é só essa oração que se examina.
# Onde não há oração de meio — «tratamento insecticida justificado quando se
# observarem posturas superiores a 3 por 100 plantas» (piralide · Friuli), ou
# «terceiro voo terminado com danos em aumento» (carpocapsa · Veneto, onde o
# «com» não é meio nenhum) — nada é bloqueado, e os dois casos que a revisão
# V1.1 verificou à mão continuam YES.
_MEIO = re.compile(
    r'(?:intervir|intervenire|intervenite|intervencao|intervento|trata\w*|'
    r'tratt\w*|defesa|difesa|cobertura|copertura|aplicar|applicare)'
    # A oracao de meio termina no ponto que FECHA a frase (ponto seguido de
    # espaco e maiuscula) ou no ponto e virgula — nunca no ponto de «Bacillus
    # t.», que e abreviatura e faria a prova sair pela metade.
    r'[^.;]{0,40}?\b(?:com|con)\b([^;]*?)(?=\.\s+[A-Z\u00c0-\u00dd]|;|\.?$)', re.I)


def _achatar(t):
    """Sem acento, minúsculo, só letras e dígitos — para comparar substância."""
    t = unicodedata.normalize('NFD', str(t or ''))
    t = ''.join(c for c in t if unicodedata.category(c) != 'Mn').lower()
    return re.sub(r'[^a-z0-9]+', '', t)


def meios_prescritos(excerto):
    """→ o trecho de MEIO que a frase prescreve, ou '' quando não prescreve."""
    m = _MEIO.search(str(excerto or ''))
    return (m.group(1) or '').strip() if m else ''


def fonte_nomeia_o_nosso(excerto, ativos):
    """A oração de meio nomeia alguma substância ativa do produto do caso?

    O confronto é por RAIZ de cinco letras achatadas, porque o boletim escreve
    em italiano e o registro em inglês: `LAMBDA-CYHALOTHRIN` × «lambda-
    cialotrina», `FLUAZINAM` × «fluazinam». Cinco letras erram para o lado
    seguro: quando a raiz não bate, o caso vai a VALIDATION_REQUIRED — que é uma
    conferência humana, não uma recusa.
    """
    meio = _achatar(meios_prescritos(excerto))
    if not meio:
        return None                       # a frase não prescreve meio nenhum
    for a in ativos:
        raiz = _achatar(a)[:5]
        if raiz and raiz in meio:
            return True
    return False


def externo(o, produtos_casados=(), ativos=()):
    ...
    if fonte_nomeia_o_nosso(o.get('NEED_EXCERPT'), ativos) is False:
        b.append('SOURCE_PRESCRIBES_OTHER_MEANS')
