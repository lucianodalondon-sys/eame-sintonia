...
PARTE C DO INGEST — a camada de SUBSTÂNCIA ATIVA que o V2.1 não tinha.
    ...
    python3 motor/v21_ingest_c.py     (roda depois de v21_ingest_b.py)
...
def main():
    ...
    for r in fut:
        ...
        fatos.append(base(
            ...
            # ⚠️ EU STATE != ITALIAN MARKETABILITY.
            # Este fato vem do EUR-Lex e prova uma data europeia. Declarar
            # GEO_ITALY junto e afirmar alcance nacional que o documento europeu
            # NAO prova — a mesma promocao de escopo que a lei
            # PROVINCIAL != REGIONAL ja proibia um nivel abaixo, e o contrato de
            # geografia (R1) reprovou 47 destes.
            #
            # A ligacao com a Italia existe e fica: e RELACAO, em
            # ITALIAN_REGISTRATIONS, nao geografia. Apontar para registros
            # italianos nao torna o fato italiano.
            r['EU_EXPIRATION_OF_APPROVAL'], [], [], ['GEO_EU'], 'EUROPEU',
            extra={
                ...
                # A LEITURA TEM DE CHEGAR A TELA, NAO SO O CAMPO.
                # O registro ja trazia NOT_A_CLAIM com a semantica certa, mas a
                # tela nao mostra campo que ela nao conhece: 47 registros
                # client-safe ficavam sem nenhuma leitura visivel, e a ressalva
                # de lei — que e justamente o que nao pode faltar — morava num
                # campo que a interface nao abre.
                'WHAT_IT_IS': (
                    'a aprovacao europeia de %s tem data-limite %s no Reg. (UE) '
                    '540/2011 consolidado. E uma data publicada, lida do EUR-Lex.'
                    % (r['ACTIVE_INGREDIENT'], r['EU_EXPIRATION_OF_APPROVAL'])),
                'WHAT_IT_PROVES': (
                    'que a substancia consta do anexo europeu com esse estado e '
                    'essa data, e que registros italianos a citam na composicao.'),
                'WHAT_IT_DOES_NOT_PROVE': (
                    'EXPIRACAO DE APROVACAO NAO E NAO-RENOVACAO: o procedimento '
                    'pode estar aberto e a data ser prorrogada. NAO e risco '
                    'regulatorio, NAO e risco comercial, NAO e oportunidade. E '
                    'ESTADO EUROPEU NAO E COMERCIALIZABILIDADE ITALIANA: o que '
                    'acontece com cada autorizacao italiana e decisao do '
                    'Ministero, e nao se deduz desta data.'),
                'GEOGRAPHY_LAW': (
                    'ESTADO EUROPEU NAO E COMERCIALIZABILIDADE ITALIANA. A data '
                    'vale para a aprovacao da substancia na UE; o que acontece '
                    'com cada autorizacao italiana e decisao do Ministero, e nao '
                    'se deduz daqui. ITALIAN_REGISTRATIONS diz quais registros '
                    'citam esta substancia — e so isso.'),
