...
class ChecarQueCheckoutNaoEColeta(unittest.TestCase):
    ...
    def setUp(self):
        env.esquecer_produzidos()
        # O acervo fica FORA disto. Esta classe escreve um RAW «alheio» para
        # fabricar a condicao que testa, e escreve-lo dentro de
        # `data/samples/` recriaria no repositorio exatamente o acervo que esta
        # missao removeu de proposito.
        self._raw = env.RAW_DIR
        env.RAW_DIR = os.path.join(tempfile.gettempdir(), 'sintonia-raw-checkout')
    ...
    def test_raw_no_disco_nao_conta_como_coleta_desta_corrida(self):
        """Há RAW no disco. Quem não colheu inventaria zero.

        A versão anterior deste teste dependia dos 23 RAW que estavam
        VERSIONADOS no repositório — e era exatamente esse acervo que fazia uma
        corrida sem coleta parecer ter colhido. Um teste que precisa da doença
        para provar a cura morre junto com a cura, e ainda impede de remover a
        doença. Agora ele FABRICA a condição: escreve um arquivo direto no
        disco, POR FORA de `guardar_raw()`, que é o que o checkout faz.

            CHECKOUT NAO E COLETA.
        """
        base = os.path.join(env.RAW_DIR, 'YOUTUBE')
        os.makedirs(base, exist_ok=True)
        alheio = os.path.join(base, 'veio-do-checkout__deadbeef.txt')
        with io.open(alheio, 'w', encoding='utf-8') as f:
            f.write('{"items": [{"id": "nao-colhido-por-esta-corrida"}]}')
        self.addCleanup(lambda: os.path.exists(alheio) and os.remove(alheio))
