...
class NenhumaCompraAcidental(Base):
    ...
    def test_A14b_o_outro_caminho_harvestapi_existe_E_ESTA_TRANCADO(self):
        ...
        Este e o achado que a missao nao nomeou, e ele nao se fecha calando-o.
        Medido: aquele ficheiro nao tem UMA referencia a `leis/social_matriz.py`
        — a politica nunca e consultada ali — e `_rodar()` leva um identificador
        de ator directo a porta paga.

            UM CAMINHO QUE NAO PERGUNTA A POLITICA NAO E PROTEGIDO PELA POLITICA.

        O que o tranca hoje e OUTRO dono: a guarda de gasto da SCRAP-SR-02, no
        unico sitio que cria execucao paga. Medido com chave no pool, para que a
        falta de token nao se confunda com uma trava:

            SemAutorizacaoDeGasto · «Ter chave, teto e rota permitida nao e ter
            autorizacao» · POSTS = 0

        Esta sentinela existe para que a tranca nao se perca em silencio. Se
        alguem lhe der uma autorizacao de gasto, a politica CONTINUA a nao ser
        perguntada ali — e isso esta registado como risco restante no artefacto,
        nao consertado por esta missao.
        """
        ...
        try:
            ...
            with self.assertRaises(ag.SemAutorizacaoDeGasto):
