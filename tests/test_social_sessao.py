...
# Cada peça vive na gaveta do que ela É. O teste segue os donos, não um caminho
# decorado: quando o SCRAP entrou na árvore canônica, `scripts/` deixou de existir.
_GAVETA_DE = {
    'social_sessao.py': 'guarda/social_sessao.py',
    'social_guarda.py': 'guarda/social_guarda.py',
    'social_rotas.py': 'coleta/social_rotas.py',
    'social_scrap.py': 'coleta/social_scrap.py',
    'social_envelope.py': 'coleta/social_envelope.py',
    'youtube_oficial.py': 'coleta/youtube_oficial.py',
    'social_matriz.py': 'leis/social_matriz.py',
    'falhas.py': 'leis/falhas.py',
}
...
class TestGuardaDeCredencial(unittest.TestCase):
    ...
    def test_o_repositorio_esta_limpo_agora(self):
        ...
        # Achados HERDADOS da árvore canônica, medidos em 2026-09-08 ao portar o
        # SCRAP para a Bíblia. NENHUM é do SCRAP e nenhum é segredo NOSSO: cinco
        # são cabeçalhos de RESPOSTA guardados como evidência de fonte pública
        # (`Set-Cookie` que o site italiano mandou), e um é um caminho pessoal
        # Windows num módulo do motor. Ficam listados AQUI, e não silenciados no
        # guarda, para que quem herdar veja o que herdou.
        #
        #     BASELINE VISÍVEL NÃO É EXCEÇÃO ESCONDIDA.
        #
        # Consertá-los é missão de quem os escreveu — não desta. O que este teste
        # continua proibindo é QUALQUER achado NOVO.
        herdados = {
            ...
            'data/samples/IT-SOURCE-SAMPLES/IT-T7-002/ELENCO-OP-AOP-al-31-12-2025-agg-08-04-2026.ods.headers.txt',
            'data/samples/ITALY-T3-005-MONITORAGGIO/headers.txt',
            'motor/v21_tm_colher.py',
        }
        nao_herdados = [n for n in novos
                        if str(n[1]).split(':')[0] not in herdados]
        self.assertEqual(nao_herdados, [],
                         'segredo ou perfil rastreado NOVO: %s' % nao_herdados)
...
class TestNaoAutentica(unittest.TestCase):
    ...
    def test_nenhum_modulo_da_missao_digita_credencial(self):
        ...
        for nome in ('social_sessao.py', 'social_rotas.py', 'social_scrap.py',
                     'social_guarda.py'):
            with open(os.path.join(RAIZ, _GAVETA_DE[nome]), encoding='utf-8') as f:
                corpo = f.read()
            for p in proibido:
