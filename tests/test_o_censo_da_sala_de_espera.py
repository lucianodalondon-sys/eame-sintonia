...
class Bancada(unittest.TestCase):
    ...
    #: Os valores que ESTES testes verificam. Nao sao a forma — sao o conteudo.
    VALORES = {"ESTADO": "PRONTO_PARA_INTELIGENCIA", "ITEM_ID": "i1",
    ...
    def unidade(self, **troca):
        """Uma unidade com a FORMA DE HOJE e os valores que o teste controla.

        ⚠️ A FORMA VEM DO DONO, E SO OS VALORES SAO ESCRITOS AQUI.
        A primeira versao escrevia as onze chaves a mao. O contrato READY
        passou a ter DEZANOVE campos — `RAW_OBSERVATION_ID`, `ESTAGIO`,
        `FACT_TIME_BASIS`, `FACT_LOCATION_BASIS`, `PUBLISHED_AT`,
        `OBSERVED_AT`, `SOURCE_DECLARED_EVIDENCE_CLASS` e `FATO` — e a unidade
        de onze campos deixou de ser «a forma de hoje». O censo mediu isso e
        contou-a como LEGADO, que e exactamente o que ela era.

            O CENSO NAO ESTAVA ERRADO: O MOLDE DESTE TESTE E QUE ERA DE ONTEM.

        E a lista escrita a mao era a MESMA falta que `_campos_do_contrato()`
        recusa cometer: uma copia do contrato que ninguem mantem. A forma passa
        a sair de `self.campos`, que vem do construtor do dono; os campos que
        este teste nao nomeia ficam `NAO SEI`, que e o que a casa escreve
        quando nao sabe — nunca vazio, nunca zero.
        """
        u = {c: "NAO SEI" for c in self.campos}
        u.update({k: v for k, v in self.VALORES.items() if k in u})
        u.update(troca)
        return u
