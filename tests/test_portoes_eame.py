...
class TestOEstadoEDerivado(unittest.TestCase):
    ...
    def test_o_portao_da_coleta_esta_partial_e_por_causa_da_localizacao(self):
        p = self.d['PORTOES']['EAME_COLLECTION_ENTRY_GATE']
        self.assertEqual('PARTIAL', p['ESTADO'])
        fams = {b['FAMILIA'] for b in p['BLOQUEADORES']}
        self.assertEqual({'LOCALIZACAO_CONFERENCIA'}, fams,
                         'apareceu bloqueador de outra familia: %s' % fams)
    ...
    def test_o_nome_do_portao_pronto_nao_promete_o_eame_inteiro(self):
        """READY num nome abrangente e lido como 'pode coletar tudo'."""
        prontos = [n for n, p in self.d['PORTOES'].items() if p['ESTADO'] == 'READY']
        for n in prontos:
            with self.subTest(portao=n):
                self.assertNotEqual('EAME_COLLECTION_ENTRY_GATE', n)
                self.assertTrue(
                    self.d['PORTOES'][n]['O_QUE_ELE_NAO_COBRE'].strip(),
...
class TestMutacoes(unittest.TestCase):
    ...
    def test_tirar_a_localizacao_da_lista_faria_o_portao_virar_ready(self):
        original = P.PORTOES['EAME_COLLECTION_ENTRY_GATE']['FAMILIAS']
        P.PORTOES['EAME_COLLECTION_ENTRY_GATE']['FAMILIAS'] = \
            [f for f in original if f != 'LOCALIZACAO_CONFERENCIA']
        try:
            d = P.monta()
            self.assertEqual('READY', d['PORTOES']['EAME_COLLECTION_ENTRY_GATE']['ESTADO'],
                             'a mutacao nao mudou nada — entao a lista nao era o que '
                             'segurava o portao, e o teste que a defende e inutil')
        finally:
            P.PORTOES['EAME_COLLECTION_ENTRY_GATE']['FAMILIAS'] = original
        # e, reposta a lista, ele volta a PARTIAL
...
class TestORawEEstrangeiro(unittest.TestCase):
    ...
    def test_as_duas_tentativas_estao_registradas(self):
        """A segunda mediu UM a menos. Guardar so a melhor seria escolher."""
