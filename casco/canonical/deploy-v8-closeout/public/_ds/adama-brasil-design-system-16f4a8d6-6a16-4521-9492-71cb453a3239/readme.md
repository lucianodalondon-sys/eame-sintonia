# ADAMA Brasil Design System

**Version:** 2.0
**Company:** ADAMA Brasil — crop protection solutions (herbicides, fungicides, insecticides)
...
Global Campaigns, August 2026. Plus the official ADAMA Brasil asset library (logos, 'A' shapes,
...
| `assets/shapes-a/` | Official 'A' shape cut-outs (transparent PNG) — 4 crops × 5 colourways × horizontal/vertical, **plus `a-shape-full.svg`** (the whole 'A' as vector — the four crops are windows on it, verified IoU 0.995–1.000) and `logo-holder-white.png` (the white corner plate that holds the logo on ADAMA Brasil pieces) |
...
| `assets/product-logos/` | ADAMA Brasil product logos, one folder per product |
...
ADAMA Brasil is a major crop-protection company (subsidiary of ADAMA Ltd.) selling to Brazilian
...
```html
<link rel="stylesheet" href="./_ds/adama-brasil-design-system/styles.css">
<script src="./_ds/adama-brasil-design-system/_ds_bundle.js"></script>
```
```js
const { Button, Badge, Tag, Card } = window.ADAMABrasilDesignSystem_16f4a8;
```
...
- **Product logos integrated.** 13 ADAMA Brasil products in `assets/product-logos/`, with the
...
- **Motion assets not included.** The After Effects projects (ADAMA Global Video Bumpers,
  lower thirds, vinhetas per product) and the rendered PNG sequences live in the ADAMA Brasil
