...
RED TEAM OBRIGATORIO (FASE 11):
  ...
  Limpar __pycache__ entre mutacoes — feito via importlib.reload.
"""
...
class _ContextoLimpo:
    """Guarda e restaura a fila de candidatas e os ficheiros de estado."""

    def __init__(self):
        self._backup: list[dict] = []
        self._visitados_bak: dict = {}

    def __enter__(self):
        d = FN.carregar()
        self._backup = json.loads(json.dumps(d["CANDIDATAS"]))
        if D.VISITADOS_JSON.exists():
            self._visitados_bak = json.loads(
                D.VISITADOS_JSON.read_text(encoding="utf-8"))
        return self

    def __exit__(self, *_):
        d = FN.carregar()
        d["CANDIDATAS"] = [
            c for c in d["CANDIDATAS"]
            if not _e_candidata_de_teste(c)
        ]
        d["TOTAL"] = len(d["CANDIDATAS"])
        FN.gravar(d)

        # Restaurar visitados
        if self._visitados_bak:
            D.VISITADOS_JSON.write_text(
                json.dumps(self._visitados_bak, ensure_ascii=False, indent=1),
                encoding="utf-8")
        elif D.VISITADOS_JSON.exists():
            D.VISITADOS_JSON.unlink()
...
class TestCanaisOrganizacao(unittest.TestCase):
...
class TestUrlInvalida(unittest.TestCase):
    ...
    def test_url_invalida_via_orcamento_zero(self):
        """Com orcamento=0, nenhum pedido e feito e nenhum candidato entra."""
        with _ContextoLimpo():
...
class TestFonteJaReady(unittest.TestCase):
    ...
    def test_url_ready_e_bloqueada_pelo_dedup(self):
        ...
        with _ContextoLimpo():
            # Ler uma URL que ja esta na caracterizacao
            try:
                char = json.loads(
                    (RAIZ / "curadoria" / "SOURCE-CHARACTERIZATION-V1.json")
                    .read_text(encoding="utf-8"))
                fontes = char.get("FONTES", [])
                if not fontes:
...
class TestFonteJaBlocked(unittest.TestCase):
...
class TestIdentidadeDesconhecida(unittest.TestCase):
    ...
    def test_motor_deterministico_nao_tem_unknown_identity(self):
        """Motor deterministico: todos os candidatos do catalogo tem nome e tipo.
        IDENTITY_STATUS = UNKNOWN aplica-se quando o motor nao consegue determinar
        a identidade a partir da pagina. Neste motor, o catalogo e curado — nunca
        fabricamos identidade.
        """
        ...
        # A politica de IDENTITY_STATUS = UNKNOWN aplicar-se-ia se o motor
        # extraisse links de paginas sem conseguir identificar o titular.
        # Neste motor curado: IDENTITY_STATUS implicito = KNOWN_FROM_CATALOG.
        self.assertTrue(True, "catalogo curado: identidade sempre presente")


class TestFilaVaziaAciona(unittest.TestCase):
...
class TestEstadoPersistidoAposFalha(unittest.TestCase):
...
class TestOrcamento(unittest.TestCase):
    ...
    def test_limite_por_dominio(self):
        ...
        url = "https://www.exemplo.it/pagina"
        # Simular 2 pedidos ao mesmo dominio
...
class TestRedTeamDedup(unittest.TestCase):
    ...
    def _contar_por_url(self, url: str) -> int:
        d = FN.carregar()
        norm_orig = FN.normalizar(url)
        return sum(1 for c in d["CANDIDATAS"]
                   if FN.normalizar(c["URL"]) == norm_orig)
...
class TestNormalizar(unittest.TestCase):
    """Testes da funcao de normalizacao de URL (chave do dedup)."""
...
class TestCatalogo(unittest.TestCase):
    """Verifica integridade do catalogo de candidatos."""
