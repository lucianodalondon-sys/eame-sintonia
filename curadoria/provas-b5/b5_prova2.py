...
SAIDA = D/"B5-PROVA-COPIA-2.json"; LOG = D/"B5-PROVA-COPIA.ndjson"
def anota(e):
...
INICIO_PROVA = datetime.fromisoformat(json.loads(open(LOG, encoding="utf-8").readline())["AT"])
_rev_orig = GD.revalidar_elegiveis
def _rev_no_relogio_simulado(agora, *, ctx=None, tarefas=None):
    # O que a prova executou aconteceu «no relogio simulado»: +8 dias nas datas das
    # tarefas mexidas desde o inicio da prova. Sem isto, a guarda anti-eco (7 dias)
    # desliga-se e as mesmas 5 fontes voltam ao topo a cada volta (1.a passagem).
    ts = []
    for t in F._ler()["TAREFAS"]:
        t = dict(t); u = t.get("UPDATED_AT")
        if u and datetime.fromisoformat(u.replace("Z", "+00:00")) >= INICIO_PROVA:
            t["UPDATED_AT"] = (datetime.fromisoformat(u.replace("Z", "+00:00")) + timedelta(days=8)).isoformat()
        ts.append(t)
    return _rev_orig(agora, ctx=ctx, tarefas=ts)
GD.revalidar_elegiveis = _rev_no_relogio_simulado
anota({"EVENTO": "PASSAGEM_2", "NOTA": "tarefas mexidas na prova datadas no relogio simulado"})
...
while voltas < 25 and len(execs) < 200:
    ...
    if not novas and not F.elegiveis() and not any(t["STATUS"] == F.WAITING_RETRY for t in F._ler()["TAREFAS"]): break
    if not novas and not F.elegiveis(): time.sleep(20)
guarda("fim")
