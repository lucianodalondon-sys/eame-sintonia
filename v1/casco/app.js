...
// O titulo era um so para todo token: "a fonte nao sustenta este campo". Mas a
// maioria destes tokens e NOSSA — NOT_APPLICABLE e a forma do nosso objeto,
// NOT_PROVED e o nosso portao, NOT_COLLECTED e a nossa coleta. So NOT_PRESENT e
// NOT_IN_SNAPSHOT falam do que a fonte traz. Culpar o Ministero por uma lacuna
// nossa e inverter a autoria da ignorancia.
const FONTE_NAO_TRAZ = ['NOT_PRESENT', 'NOT_IN_SNAPSHOT'];
const val = v => isUnk(v)
  ? `<span class="unknown" title="${FONTE_NAO_TRAZ.includes(String(v))
      ? 'a fonte oficial nao traz valor neste campo'
      : 'token de ignorancia desta ferramenta: nada aqui sustenta um valor'}">${esc(v)}</span>`
  : esc(v);
...
// A glosa e obrigatoria ONDE O SELO APARECE, nao so nas telas que a listam:
// ACT_NOW estava sendo impresso sem ela nas fichas de PRODUTO 360, na BUSCA e
// na gaveta de evidencia.
const janelaDe = o => (HOJE ? janelaAgora(o)[0] : o.TIME_WINDOW);
const glosaSe = lista => lista.some(o => janelaDe(o) === 'ACT_NOW') ? GLOSA_JANELA : '';
...
function janelaSelo(o) {
  const [j, r] = HOJE ? janelaAgora(o) : [o.TIME_WINDOW, o.TIME_WINDOW_RULE];
...
// X-08 · os 210 objetos carregam 201 identificadores distintos: 7 colisoes, e
// todas em NEEDS_HUMAN_REVIEW. Esta funcao resolvia com .find(), que devolve o
// PRIMEIRO — e 5 das 13 linhas da fila abriam a gaveta de OUTRO objeto,
// apontando outra pagina do mesmo rotulo. O botao prometia "esta e a prova
// desta linha" e entregava a de outra, que e exatamente a promessa central da
// ferramenta falhando. A tela passa a resolver por POSICAO no payload, que e
// exata, e a gaveta DECLARA a colisao onde ela existe, em vez de escondê-la.
function evIdx(i) {
  const o = P.objects[i];
  if (!o) return;
  ...
  <div class="meta">objeto <code>${esc(o.INTELLIGENCE_OBJECT_ID)}</code>${(() => {
    const n = P.objects.filter(x => x.INTELLIGENCE_OBJECT_ID === o.INTELLIGENCE_OBJECT_ID).length;
    return n > 1 ? ` &middot; <span class="unknown">INTELLIGENCE_OBJECT_ID_NOT_UNIQUE</span>
      <div class="meta"><b>${n} objetos deste build compartilham este identificador.</b> Esta
      gaveta e a linha que voce clicou, resolvida por posicao no payload; o identificador,
      sozinho, nao a distingue. Isto e um defeito do gerador de id, e esta escrito aqui em vez
      de corrigido em silencio.</div>` : '';})()}</div>
  <dl>
    <dt>Fato (o que a fonte diz)</dt><dd>${fatoDemo(o)}</dd>
    ...
    <dt>Antes &rarr; Depois</dt><dd>${valorDemo(o)} &rarr; ${depoisDemo(o)}</dd>
    ...
    <dt>Local da evidencia</dt><dd>${localDemo(o)}</dd>
    ...
    <dt>Capturado em</dt><dd>${val(o.CAPTURED_AT)}${o.OBJECT_TYPE === 'EXPIRY_EVENT'
      ? `<div class="meta">Nada foi capturado nesta data. Este objeto nao nasce de uma coleta
         propria: e derivado do instantaneo nomeado acima
         (<code>${esc(String(o.SOURCE_DOCUMENT_AFTER).split(' ')[0])}</code>), e a data marca
         <b>quando a derivacao rodou</b>. <code>CAPTURED_AT != EFFECTIVE_AT</code>, e aqui
         tambem <code>CAPTURED_AT != a data da captura</code>.</div>` : ''}</dd>
    ...
    <dt>Janela temporal</dt><dd>${janelaSelo(o)}
      <div class="meta">regra no build: <code>${esc(o.TIME_WINDOW_RULE)}</code>${HOJE
        ? ` &middot; recalculada contra ${esc(HOJE)} pela regra <code>${esc(janelaAgora(o)[1])}</code>`
...
window.evIdx = evIdx;
// Mantida para quem chama por identificador. Onde o identificador e ambiguo ela
// nao tem como acertar, e a gaveta diz isso.
function evObj(id) { evIdx(P.objects.findIndex(x => x.INTELLIGENCE_OBJECT_ID === id)); }
window.evObj = evObj;
...
// ---------------------------------------------------------------- 1 · TODAY
function cardObj(o) {
  const [jAgora, jRegra] = HOJE ? janelaAgora(o) : [o.TIME_WINDOW, o.TIME_WINDOW_RULE];
  ...
  return `<div class="card ${cls}">
    ...
    <div class="meta">${fatoDemo(o)}</div>
    <div class="ba"><span class="b">${valorDemo(o)}</span>
      <span class="arr">&rarr;</span><span class="a">${depoisDemo(o)}</span></div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:6px">
      ...
      <span class="meta">detectado ${isUnk(o.DETECTED_AT)?val(o.DETECTED_AT):esc(dISO(o.DETECTED_AT))}</span>
      <button class="ev" onclick="evIdx(${P.objects.indexOf(o)})">ver evidencia</button>
    </div>
...
// ---------------------------------------------------------------- 0 · VISAO GERAL
// A home da demo. Quatro perguntas, nenhuma acao inventada. Todo numero aqui e
// recontado do payload no momento em que a tela abre — nao ha numero herdado de
// documento nem escrito a mao no codigo.
let AREA_HOME = 'REGULATORY';
function setArea(a) { AREA_HOME = a; viewHome(); }
window.setArea = setArea;

function viewHome() {
  const prov = P.objects.filter(o => o.PROOF_STATE === 'PROVED');
  const mud = prov.filter(o => o.OBJECT_TYPE !== 'EXPIRY_EVENT');
  ...
  const venc90 = P.products.filter(p => { const d = dte(p); return typeof d === 'number' && d >= 0 && d <= 90; });
  const conflito = P.products.filter(conflitoDeValidade);
  const nUsos = P.products.reduce((n,p) => n + (p.uses||[]).length, 0);
  const nDoses = P.products.reduce((n,p) => n + (p.doses||[]).length, 0);

  const daArea = P.objects.map(o => {
    const r = (o.CAPABILITY_ROUTING || []).find(x => x.CAPABILITY_ID === AREA_HOME);
    return r ? {o, r} : null;
  }).filter(Boolean);
  const relev = daArea.filter(x => x.r.ROUTING_STATE === 'RELEVANT' || x.r.ROUTING_STATE === 'POTENTIALLY_RELEVANT');
  const semRegra = daArea.filter(x => x.r.ROUTING_STATE === 'UNKNOWN');
  const barrados = daArea.filter(x => x.r.ROUTING_STATE === 'NOT_RELEVANT');

  $('#v-home').innerHTML = `
  <div class="lei"><b>O que esta ferramenta faz, em uma frase:</b> ela acompanha a fonte oficial
    italiana, compara as versoes do registro, separa mudanca verdadeira de ruido tecnico, mostra
    quem pode precisar olhar, e leva toda afirmacao de volta ao documento oficial.
    <b>Quando nao consegue provar, ela nao inventa: manda para revisao.</b></div>
  ${avisoRelogio()}

  <div class="cards">
    <div class="kpi"><b>${mud.length}</b><span>mudancas provadas na janela observada</span></div>
    <div class="kpi"><b style="color:var(--bad)">${conflito.length}</b><span>validade vencida e registro ainda em vigor</span></div>
    <div class="kpi"><b style="color:var(--warn)">${venc90.length}</b><span>vencem nos proximos 90 dias</span></div>
    <div class="kpi"><b style="color:var(--rev)">${rev.length + dq.length}</b><span>itens que a maquina recusou responder</span></div>
    <div class="kpi"><b class="mono" style="font-size:15px">${esc(P.history.window)}</b><span>janela observada</span></div>
  </div>

  <div class="block">
    <h3>1 &middot; O que mudou</h3>
    <div class="meta"><b>${P.history.snapshots}</b> instantaneos oficiais baixados, dos quais
      <b>${P.history.distinct}</b> sao documentos distintos por sha256 (os outros
      ${P.history.snapshots - P.history.distinct} eram republicacao identica, e republicar nao e
      versao nova). Comparando os ${P.history.distinct} documentos: <b>${P.history.raw_field_diffs}</b>
      diferencas brutas de campo &rarr; <b>${P.history.noise}</b> (${P.history.noise_pct}%) eram a
      fonte reordenando a propria lista de indicacoes de perigo e nao viram evento &rarr; restam
      <b>${P.history.normalised_field_diffs}</b> diferencas de campo reais. Somando
      <b>${P.by_type.PRODUCT_ENTERED_REGISTRY || 0}</b> produtos que entraram no registro, sao
      <b style="color:var(--ok)">${P.history.true_changes}</b> eventos regulatorios.
      <b>Sem esse filtro seriam ${P.history.raw_field_diffs + (P.by_type.PRODUCT_ENTERED_REGISTRY||0)}
      alertas para encontrar ${P.history.true_changes}</b> &mdash; as
      ${P.history.raw_field_diffs} diferencas brutas de campo <b>mais</b> as
      ${P.by_type.PRODUCT_ENTERED_REGISTRY||0} entradas de produto, que a contagem de diferenca
      de campo nao inclui. Comparar ${P.history.raw_field_diffs} com ${P.history.true_changes}
      seria comparar dois universos.</div>
    <div style="margin-top:8px">
      <button class="ev" onclick="go('today')">ver as mudancas, uma a uma, com antes e depois</button>
      <button class="ev" onclick="go('timeline')">ver as ${P.versions.length} versoes do registro</button>
    </div>
  </div>

  <div class="block">
    <h3>2 &middot; O que precisa de atencao <span class="meta">&mdash; por area</span></h3>
    <div style="margin-bottom:8px">${Object.keys(CAPS).map(c =>
      `<button class="ev" style="${AREA_HOME===c?'outline:2px solid var(--ok)':''}"
        onclick="setArea('${c}')">${CAPS[c]}</button>`).join(' ')}</div>
    <div class="meta">
      <b>${relev.length}</b> objeto(s) roteados para <b>${CAPS[AREA_HOME]}</b> por regra escrita
      ${semRegra.length?` &middot; <span class="unknown">${semRegra.length} sem regra (UNKNOWN)</span>`:''}
      ${barrados.length?` &middot; ${barrados.length} barrado(s) por portao`:''}
    </div>
    ${relev.length ? `<div class="tw" style="margin-top:8px"><table>
      <thead><tr><th>Produto</th><th>O que mudou</th><th>Antes &rarr; Depois</th>
        <th>Detectado em</th><th>Estado</th><th>Regra</th><th></th></tr></thead>
      <tbody>${relev.slice(0,12).map(({o,r}) => `<tr>
        <td>${esc(o.PRODUCT_NAME||'NOT_KNOWN')}<div class="mono meta">${esc(o.REGISTRATION_ID)}</div></td>
        <td>${esc(o.CHANGE_TYPE)}</td>
        <td>${valorDemo(o)} &rarr; ${depoisDemo(o)}</td>
        <td class="mono">${esc(dISO(o.DETECTED_AT))}
          <div class="meta">nao e o valor de hoje: e o que mudou entre dois instantaneos naquela
          data. O valor vigente esta na ficha do produto.</div></td>
        <td><span class="pill ${r.ROUTING_STATE==='RELEVANT'?'p-ok':'p-warn'}">${esc(r.ROUTING_STATE)}</span></td>
        <td class="meta"><code>${esc(r.RULE_ID)}</code></td>
        <td><button class="ev" onclick="evIdx(${P.objects.indexOf(o)})">evidencia</button></td>
      </tr>`).join('')}</tbody></table></div>
      ${relev.length>12?`<div class="meta">mostrando 12 de ${relev.length} &mdash;
        <button class="ev" onclick="go('action')">ver todos por area</button></div>`:''}`
      : `<div class="lei" style="border-left-color:var(--unk)">
         <b>Esta area nao recebe nenhum objeto nesta versao, e isso e uma resposta, nao uma falha.</b>
         ${AREA_HOME==='COMMERCIAL_RTV'
           ? `A regra <code>C-05</code> barra fato regulatorio bruto de chegar ao campo: os
              ${barrados.length} objetos ficam <code>NOT_RELEVANT</code> ate o portao
              <code>G-01</code>, que exige prova e revisao humana registrada. Esta versao nao abre
              esse portao.`
           : `Nenhuma regra escrita roteia estes tipos de evento para ca. Os ${semRegra.length}
              objetos saem como <code>UNKNOWN</code>, que significa <b>nao sabemos</b>, e nao
              &ldquo;nao interessa&rdquo;.`}</div>`}
    <div class="meta" style="margin-top:8px"><b>A ferramenta nao emite acao.</b> Ela roteia por
      regra escrita e para em <code>REVIEW_RECOMMENDED</code>. Quem decide e uma pessoa.</div>
  </div>

  <div class="block">
    <h3>3 &middot; O que esta em revisao <span class="meta">&mdash; a maquina recusando responder</span></h3>
    <div class="lei">Isto e funcionalidade, nao defeito. <b>${rev.length}</b> doses foram
      rebaixadas porque a leitura nao se sustentou contra o documento, e <b>${dq.length}</b>
      rotulos tiveram a tabela de DOSE nao lida. Nenhum deles vira numero na tela.
      <code>PARSER_FAILURE != REGULATORY_ABSENCE</code>: nao ter lido nao e o produto nao ter.</div>
    <div style="margin-top:8px"><button class="ev" onclick="go('review')">ver a fila de revisao</button>
      <button class="ev" onclick="go('cov')">ver a cobertura, etapa por etapa</button></div>
  </div>

  <div class="block">
    <h3>4 &middot; O que vence ou muda no tempo</h3>
    <div class="meta"><b>${conflito.length}</b> produtos com a validade vencida e o registro ainda
      declarando estado em vigor — conflito entre dois campos oficiais do mesmo instantaneo, nao
      conclusao nossa. <b>${venc90.length}</b> vencem nos proximos 90 dias.
      <b>Vencer nao e ser revogado:</b> a ferramenta mostra os dois campos e nao conclui saida de
      mercado.</div>
    <div style="margin-top:8px"><button class="ev" onclick="go('cal')">ver o calendario</button></div>
  </div>

  <div class="block" style="border-left:3px solid var(--warn)">
    <h3>O que esta deliberadamente fora desta demonstracao</h3>
    <div class="meta">Cultura &times; alvo e dose foram <b>retiradas</b>. O leitor extraiu
      ${nUsos} pares e ${nDoses} linhas de dose deste acervo, e uma terceira rodada adversarial
      independente ainda esta julgando se a atribuicao de cultura sobrevive a geometria da tabela.
      Ate ela fechar, nada dessa camada aparece como fato.
      <b>Esconder uma funcao e melhor do que demonstrar uma informacao regulatoria errada.</b></div>
  </div>`;
}
...
function viewToday() {
  ...
  const mudancas = provados.filter(o => o.OBJECT_TYPE !== 'EXPIRY_EVENT').sort((a,b) =>
    (ordem[(HOJE?janelaAgora(a)[0]:a.TIME_WINDOW)]??9) - (ordem[(HOJE?janelaAgora(b)[0]:b.TIME_WINDOW)]??9) ||
  ...
  <div class="lei">Entre os <b>${P.history.distinct}</b> documentos distintos:
    <b>${P.history.raw_field_diffs}</b> diferencas brutas de campo,
    <b>${P.history.noise}</b> (${P.history.noise_pct}%) de reordenacao da propria fonte,
    <b>${P.history.normalised_field_diffs}</b> diferencas de campo reais, mais
    <b>${P.by_type.PRODUCT_ENTERED_REGISTRY || 0}</b> produtos novos =
    <b>${P.history.true_changes}</b> eventos na janela inteira observada.
  ...
  <div class="tw"><table>
    ...
    <tbody>${condicoes.map(o => {
      ...
      <td>${val(p2.status)}</td>
      <td><button class="ev" onclick="evIdx(${P.objects.indexOf(o)})">evidencia</button></td></tr>`;
...
function viewProduto(reg) {
  ...
  <div class="block">
    ...
    <div class="tw" style="margin-top:9px"><table><tbody>
      ...
      <tr><th>Validade</th><td${venc && !foraDeVigor(p)?' style="color:var(--bad)"':''}>${val(p.expiry)}
        ${venc ? (foraDeVigor(p)
          ? ` &mdash; vencida ha ${-D} dias, e o estado declarado <b>ja e
             &ldquo;${esc(p.status)}&rdquo;</b>: os dois campos oficiais <b>concordam</b>, nao ha
             conflito a reportar.`
          : ` &mdash; vencida ha ${-D} dias e o registro ainda o lista como
             &ldquo;${esc(p.status)}&rdquo;, que e um estado em vigor.
  ...
  <div class="block" style="border-left:3px solid var(--warn)">
    <h3>Usos autorizados e dose <span class="meta">&mdash; retidos nesta demonstracao</span></h3>
    ${(() => {
      // X-03 · o ramo unico afirmava "o sistema leu esta informacao e esta se
      // recusando a publica-la ... sao 0 pares e 0 linhas" em 35 fichas. Tres
      // frases falsas numa so: nada foi lido, nada esta sendo retido, e e
      // exatamente falta de dado. O fork agora e sobre o que foi MEDIDO, e a
      // frase de recusa so aparece onde existe leitura a reter.
      const st = p.states || {};
      const nu = (p.uses||[]).length, nd = (p.doses||[]).length;
      if (st.LABEL_DOWNLOADED === false) return `<div class="lei">
        <b>Nao ha documento para ler.</b> Nenhum rotulo foi baixado para este registro
        (<span class="unknown">NOT_COLLECTED</span>), entao nao existe par nem linha de dose a
        reter: nao e recusa, e ausencia de coleta.</div>`;
      // F-06 · os dois SPURIOUS_TABLE_DISCARDED sao a recusa mais literal do
      // build e nao apareciam em tela nenhuma.
      if (p.dose_state === 'SPURIOUS_TABLE_DISCARDED') return `<div class="lei">
        <b>O leitor achou uma tabela onde nao havia, e ela foi descartada.</b> Estado
        <code>SPURIOUS_TABLE_DISCARDED</code>, regra <code>P-01</code>: o que voltou nao tinha
        forma de linha de dose, e um palpite descartado nao vira dado. Nao ha dose retida aqui
        porque nao ha dose lida. Pares cultura &times; alvo extraidos: <b>${nu}</b> &mdash;
        tambem nao publicados nesta demonstracao.</div>`;
      if (!nu && !nd) return `<div class="lei">
        <b>O leitor rodou neste documento e estruturou zero.</b> Isto e medicao, nao ignorancia e
        nao recusa: o rotulo foi baixado e lido, e o leitor de uso devolveu <b>0</b> pares e o de
        dose <b>0</b> linhas. <b>Nao ha nada retido nesta ficha.</b>
        <div class="meta" style="margin-top:6px">E nada aqui afirma que este produto nao tem uso
        ou nao tem dose. A maioria dos herbicidas italianos declara dose em prosa
        (&ldquo;alla dose di 1-3 l/ha&rdquo;) e este leitor le tabela.
        <code>PARSER_FAILURE != REGULATORY_ABSENCE</code></div></div>`;
      return `<div class="lei">
        <b>O sistema leu esta informacao e esta se recusando a publica-la.</b>
        Nao e falta de dado: sao ${nu} pares cultura &times; alvo e ${nd} linhas de dose que o
        leitor extraiu deste documento.
      <div class="meta" style="margin-top:6px">A recusa e deliberada e tem uma razao medida. O
      leitor de uso reusado atribui a linha a cultura por proximidade vertical, e isso falha onde
      a tabela tem <b>fios desenhados</b> separando blocos de cultura &mdash; ha caso medido em
      que a linha atribuida a uma cultura pertence, na tabela real, ao bloco da cultura de cima,
      do outro lado de um fio. Enquanto uma terceira rodada adversarial independente nao
      adjudicar essa camada, <b>nenhum par cultura &times; alvo e nenhuma dose sao apresentados
      como fato</b> nesta demonstracao.</div>
      <div class="meta" style="margin-top:6px">Uma cultura autorizada errada e um fato regulatorio
      errado <b>mesmo sem numero ao lado</b>. Por isso a camada inteira sai, e nao apenas a dose.
      <b>Esconder uma funcao e melhor do que demonstrar uma informacao regulatoria errada.</b></div>
      ${st.NEEDS_REVIEW ? `<div class="meta" style="margin-top:6px">Estado:
      <span class="unknown">NEEDS_REVIEW</span> &middot; adjudicacao em curso &middot; o que a
      ferramenta ja consegue provar sobre este produto esta acima, e volta a fonte oficial.</div>`
      : `<div class="meta" style="margin-top:6px">Este registro <b>nao</b> esta em revisao:
      nenhuma leitura dele foi rebaixada por regra. A retencao acima e da demonstracao inteira, e
      nao um estado deste produto &mdash; o estado de revisao vale para
      ${P.products.filter(x => x.states && x.states.NEEDS_REVIEW).length} dos
      ${P.products.length} registros, e este nao e um deles.</div>`}</div>`;
    })()}
  </div>

  <div class="block">
    <h3>Tempo de carencia (PHI)</h3>
    <div class="lei">Nao publicado nesta versao, e nao por falta de tentativa. O extrator de
      carencia esta marcado <code>PROTOTYPE_NOT_SHIPPED</code> e foi medido sobre <b>15</b> dos
      ${P.coverage.PHI_COVERAGE.OF} rotulos: 2 com carencia lida, 8 com a secao localizada e
      nenhuma linha extraida, 5 sem secao &mdash; e a primeira linha de cada bloco vinha
      contaminada pela coluna vizinha. <b>As etichette trazem carencia; nos e que nao lemos.</b>
      Nada disso alimenta esta ficha: <code>PHI_PUBLISHED = 0</code>.</div>
  </div>
...
function viewTimeline() {
  const v = P.versions;
  // F-10 · "republicado sem mudar" so vale quando o instantaneo IMEDIATAMENTE
  // anterior e o mesmo documento. Duas das seis repeticoes nao sao isso: entre
  // elas e a versao que repetem ha um instantaneo com sha diferente. A fonte
  // mudou e voltou ao conteudo anterior — retorno, nao republicacao. O efeito
  // regulatorio e o mesmo (nenhum), mas o nome nao pode apagar a oscilacao.
  const SHA_DE = {};
  v.forEach(x => { SHA_DE[x.date] = x.id; (x.republished||[]).forEach(d => SHA_DE[d] = x.id); });
  const DATAS = Object.keys(SHA_DE).sort();
  const imediata = d => { const i = DATAS.indexOf(d);
    return i > 0 && SHA_DE[DATAS[i-1]] === SHA_DE[d]; };
  const retornos = DATAS.filter(d => SHA_DE[d] !== d && !imediata(d));
  const porData = {};
  ...
  <div class="cards">
    ...
    <div class="kpi"><b style="color:var(--dim)">${P.history.snapshots - P.history.distinct}</b><span>instantaneos que repetem um documento ja visto (nao contam como versao)</span></div>
  ...
  ${retornos.length ? `<div class="lei" style="border-left-color:var(--unk)">
    <b>Dos ${P.history.snapshots - P.history.distinct} instantaneos que repetem um documento ja
    visto, ${retornos.length} nao sao republicacao.</b> Em
    ${retornos.map(d => `<code>${esc(d)}</code>`).join(' e ')} o conteudo volta a ser o de uma
    versao anterior, mas entre as duas ha um instantaneo com sha256 <b>diferente</b>: a fonte
    mudou e voltou. O efeito regulatorio e o mesmo &mdash; nenhum evento nasce dai &mdash; mas
    chamar retorno de republicacao apagaria uma oscilacao que o registro realmente teve.</div>` : ''}
  ...
  <div class="tw"><table>
    ...
    <tbody>${v.map((x,i) => {
      ...
      return `<tr>
        ...
        <td class="meta">${x.republished.length
          ? x.republished.map(d => imediata(d) ? esc(d)
              : `${esc(d)} <span class="unknown" title="entre esta data e a versao que ela repete ha um instantaneo com sha256 diferente: a fonte mudou e voltou">RETORNO_APOS_MUDANCA</span>`).join(', ')
...
// R-01/R-11 · o texto FACT dos objetos NEEDS_HUMAN_REVIEW imprime o triplo
// cultura x alvo x dose em prosa, e por isso publicava a camada retirada em
// quatro superficies (ficha, busca, gaveta, por area) — inclusive um valor
// ERRADO para o par: "Cavolo cappuccio x Cimici ... 280-600" quando a etichetta
// 008259 da 560-1000 a esse par. A demo troca o TEXTO, nao o objeto.
const RETIDO_TXT = '<span class="unknown">WITHHELD_PENDING_ADJUDICATION</span>';
function fatoDemo(o) {
  // X-09 · o FACT gravado diz "o leitor de tabela devolveu NO_USE_TABLE_FOUND",
  // debaixo de um cabecalho que promete "o que a FONTE diz". Quem produziu o
  // estado foi o leitor de DOSE (PARSER_VERSION = v1/dose_extrair no proprio
  // objeto), e a frase descreve NOS, nao a etichetta. Em 91 fichas ela vinha
  // seis linhas abaixo de "sao N pares cultura x alvo" com N ate 157.
  if (o && o.OBJECT_TYPE === 'DATA_QUALITY_EVENT') return `o leitor de <b>dose</b>
    (<code>${esc(o.PARSER_VERSION)}</code>) devolveu ${estadoLeitorDose(o) || val(o.AFTER_VALUE)}
    ao ler este documento.
    <div class="meta">Isto e um estado do <b>nosso leitor</b>, nao uma afirmacao da etichetta.
    <code>PARSER_FAILURE != REGULATORY_ABSENCE</code></div>`;
  if (!o || o.OBJECT_TYPE !== 'NEEDS_HUMAN_REVIEW') return esc(o && o.FACT);
  return `${RETIDO_TXT}
    <div class="meta">uma leitura de dose foi rebaixada neste produto. A cultura, o alvo e o valor
    lidos ficam fora desta demonstracao: sao a camada que a terceira rodada adversarial ainda
    julga. O que a demo afirma aqui e apenas <b>que houve recusa</b>, e por qual mecanismo.</div>`;
}
function valorDemo(o) {
  return (o && o.OBJECT_TYPE === 'NEEDS_HUMAN_REVIEW') ? RETIDO_TXT : val(o && o.BEFORE_VALUE);
}
// X-01 · valorDemo() cobria so a coluna ANTES, e so nas duas telas onde foi
// chamada. Os cards de evento e a gaveta seguiam imprimindo val(o.BEFORE_VALUE)
// e val(o.AFTER_VALUE) crus: o mesmo "280-600" que o portao ja tinha reprovado,
// mudado de superficie. Retirar um claim de uma tela nao e retirar o claim.
// Agora as DUAS colunas passam por aqui, e nenhum ramo imprime o valor direto.
function depoisDemo(o) {
  if (o && o.OBJECT_TYPE === 'NEEDS_HUMAN_REVIEW') return RETIDO_TXT;
  return estadoLeitorDose(o) || val(o && o.AFTER_VALUE);
}
// X-09 · NO_USE_TABLE_FOUND e TABLE_FOUND_NO_ROWS sao estados do leitor de
// DOSE (PARSER_VERSION = v1/dose_extrair no proprio objeto). O nome carrega a
// palavra USE por historia do codigo. Impresso nu numa coluna de mudanca, ao
// lado de uma ficha que diz "sao N pares cultura x alvo" com N ate 157, ele le
// como "este rotulo nao tem uso" — que e PARSER_FAILURE lido como
// REGULATORY_ABSENCE. O token continua visivel, porque e o que o payload
// guarda; o que ele nunca mais faz e aparecer sozinho.
const EST_DOSE = ['NO_USE_TABLE_FOUND', 'TABLE_FOUND_NO_ROWS', 'SPURIOUS_TABLE_DISCARDED'];
function estadoLeitorDose(o) {
  const v = String(o && o.AFTER_VALUE);
  if (!o || o.OBJECT_TYPE !== 'DATA_QUALITY_EVENT' || !EST_DOSE.includes(v)) return '';
  return `<span class="unknown">${esc(v)}</span> <span class="meta">&mdash; estado do leitor de
    <b>dose</b> (<code>v1/dose_extrair</code>), nao do leitor de uso</span>`;
}
// X-07 · o mesmo erro escrito em prosa: EVIDENCE_LOCATION terminava em
// "nenhuma tabela de uso localizada" nos 140 objetos, debaixo do cabecalho
// FATO (O QUE A FONTE DIZ) — e em 18 deles o campo ao lado diz
// TABLE_FOUND_NO_ROWS, isto e, a tabela FOI encontrada. A clausula sai.
function localDemo(o) {
  if (!o || o.OBJECT_TYPE !== 'DATA_QUALITY_EVENT') return val(o && o.EVIDENCE_LOCATION);
  const t = String(o.EVIDENCE_LOCATION || '').replace(/,?\s*nenhuma tabela de uso localizada\s*$/, '');
  return `${esc(t)}<div class="meta">o leitor percorreu o documento inteiro. ${
    String(o.AFTER_VALUE) === 'TABLE_FOUND_NO_ROWS'
      ? 'Neste objeto a tabela <b>foi encontrada</b> e nenhuma linha foi lida dela.'
      : 'O que ele nao localizou foi a tabela de <b>dose</b>.'}
    Isto descreve o <b>nosso leitor</b>, nao a etichetta.</div>`;
}
...
// ---------------------------------------------------------------- 4 · CROP x TARGET
// A tela CULTURA x ALVO foi REMOVIDA desta build de demonstracao, e nao apenas
// escondida do menu: codigo morto que ainda sabe renderizar dose e um risco de
// vazamento, nao uma conveniencia.
...
function viewCal() {
  ...
  const bloco = (lo,hi,lbl) => {
    ...
    ${l.length ? `<div class="tw"><table>
      <thead><tr><th>Validade</th><th>Faltam</th><th>Produto</th><th>Registro</th><th>Estado</th><th></th></tr></thead>
  ...
  ${vencCoerente.length ? `<div class="block">
    ...
    <div class="lei" style="border-left-color:var(--unk)"><b>Este numero e contado sobre um
      universo pequeno, e le mal sozinho.</b> Os dois blocos desta tela percorrem os
      ${P.products.length} registros com ficha nesta ferramenta. O conjunto ativo do instantaneo
      &mdash; os ${P.coverage.LABEL_DISCOVERY_COVERAGE.OF} produtos que a coleta percorreu &mdash;
      exclui por construcao quase tudo o que ja saiu de vigor: no instantaneo vigente ha
      <b>602</b> linhas ADAMA, das quais <b>439</b> ja declaram estado fora de vigor
      (<code>Revocato</code> 425, <code>Scaduto</code> 14). <b>Nao leia
      &ldquo;${vencCoerente.length}&rdquo; como &ldquo;so ${vencCoerente.length} produtos ADAMA
      estao vencidos e fora de vigor&rdquo;</b>: os outros nao estao aqui porque nao foram
      coletados, e ausencia de coleta nao e ausencia.</div>
    <div class="tw" style="margin-top:8px"><table>
      <thead><tr><th>Validade</th><th>Ha</th><th>Produto</th><th>Registro</th><th>Estado declarado</th><th></th></tr></thead>
...
function tabRev(lista) {
  return `<div class="tw"><table>
    <thead><tr><th>Produto</th><th colspan="2">Leitura rebaixada</th><th>Valor</th>
    ...
    <tbody>${lista.map(o => `<tr>
      ...
      <td colspan="2" class="meta"><span class="unknown">WITHHELD_PENDING_ADJUDICATION</span>
        <div class="meta">a leitura foi rebaixada e <b>nao e exibida</b> nesta demonstracao: e a
        camada que a terceira rodada adversarial ainda julga, e mostrar o triplo aqui daria a ele
        a aparencia de fato.${isUnk(o.BEFORE_VALUE) ? ` <b>Nesta linha nao ha valor lido a
        reter</b>: o campo chegou como <span class="unknown">${esc(o.BEFORE_VALUE)}</span>. Dizer
        que ele &ldquo;existe e esta no dossie&rdquo; seria afirmar posse de um numero que nunca
        foi lido — a frase valia para a maioria das linhas e era falsa nestas.` : ''}</div></td>
      <td style="color:var(--bad)">${val('NOT_PROVED')}</td>
      <td class="meta">${val(o.DEMOTION_RULE)}</td>
      <td class="meta">${o.DEMOTION_MECHANISM==='DRAWN_TABLE_RULE'
        ? `<b>CONTRADICTED_BY_RULE</b><div class="meta">o fio desenhado contradisse o valor — e por
           isso que esta linha esta aqui</div>`
        : o.RULE_CHECK_BEFORE_DEMOTION==='CONFIRMED_BY_RULE'
...
function viewReview() {
  ...
  const dq = P.objects.filter(o => o.OBJECT_TYPE === 'DATA_QUALITY_EVENT');
  // R-07 · os KPIs denominavam sobre 166 e contavam como "medido zero" os 3
  // registros para os quais NENHUM rotulo foi baixado. Ausencia de coleta nao e
  // ausencia medida, e as duas nao podem somar no mesmo numero.
  const comRotulo = P.products.filter(p => p.states && p.states.LABEL_DOWNLOADED);
  const semColeta = P.products.filter(p => p.states && p.states.LABEL_DOWNLOADED === false);
  const semUsoMedido = comRotulo.filter(p => !p.uses.length);
  const semDoseMedido = comRotulo.filter(p => !p.doses.length);
  $('#v-review').innerHTML = `
  ...
  <div class="cards">
    ...
    <div class="kpi"><b style="color:var(--unk)">${dq.length}</b><span>rotulos sem tabela de dose lida</span></div>
    <div class="kpi"><b style="color:var(--unk)">${semUsoMedido.length}</b><span>produtos <b>com rotulo lido</b> e zero pares</span></div>
    <div class="kpi"><b style="color:var(--unk)">${semDoseMedido.length}</b><span>produtos <b>com rotulo lido</b> e zero dose</span></div>
    <div class="kpi"><b style="color:var(--unk)">${semColeta.length}</b><span>registros sem rotulo coletado</span></div>
  </div>
  <div class="meta" style="margin:-4px 0 10px">Os dois numeros do meio contam coisas diferentes e
    a diferenca tem nome: <b>${dq.length}</b> e o numero de <b>objetos</b> emitidos pelo leitor de
    dose; <b>${semDoseMedido.length}</b> e o numero de <b>produtos com rotulo lido e zero linha de
    dose</b>. Eles diferem por ${semDoseMedido.length - dq.length}, e os
    ${semDoseMedido.length - dq.length} sao os produtos abaixo: o leitor achou tabela onde nao
    havia e ela foi descartada, entao nao houve objeto a emitir.</div>
  ${(() => { const esp = P.products.filter(p => p.dose_state === 'SPURIOUS_TABLE_DISCARDED');
    if (!esp.length) return '';
    return `<h2>Tabela descartada porque nao era tabela de dose (${esp.length})</h2>
    <div class="lei"><b>Esta e a recusa mais literal deste build, e ate agora era a unica
      invisivel.</b> Regra <code>P-01</code>: o extrator devolveu algo com forma de tabela, o
      filtro olhou e concluiu que aquilo nao eram linhas de dose. O palpite foi descartado, e um
      palpite descartado nao emite objeto — por isso estes dois produtos nao apareciam em tela
      nenhuma, numa tela que se anuncia como &ldquo;o que a maquina recusou adivinhar&rdquo;.</div>
    <div class="tw"><table>
      <thead><tr><th>Produto</th><th>Registro</th><th>Estado do leitor de dose</th>
        <th>Pares extraidos <span class="meta">(em adjudicacao, nao publicados)</span></th>
        <th>Linhas de dose</th><th></th></tr></thead>
      <tbody>${esp.map(p => `<tr>
        <td>${esc(p.name)}</td><td class="mono">${esc(p.reg)}</td>
        <td><code>${esc(p.dose_state)}</code></td>
        <td>${contagem(p,'uses','LABEL_READ')}</td>
        <td><b>0</b> <span class="meta">medido: a tabela lida foi descartada</span></td>
        <td><button class="ev" onclick="go('produto');viewProduto('${esc(p.reg)}')">ver ficha</button></td>
      </tr>`).join('')}</tbody></table></div>`; })()}
  ...
  <h2>Rotulos cuja tabela de DOSE nao foi lida (${dq.length})</h2>
  <div class="meta" style="margin-bottom:6px">Estes ${dq.length} objetos sao estados do leitor de
    <b>dose</b> (<code>v1/dose_extrair</code>), nao do leitor de uso: a coluna ao lado mostra que a
    maioria deles TEM pares extraidos. Chamar isto de &ldquo;tabela de uso nao lida&rdquo; era
    descrever o leitor errado.</div>
  ...
  <div class="tw"><table>
    <thead><tr><th>Produto</th><th>Registro</th><th>Atividade</th><th>Estado do leitor</th>
      <th>Pares extraidos <span class="meta">(em adjudicacao, nao publicados)</span></th><th></th></tr></thead>
    <tbody>${dq.slice(0,200).map(o => {
      ...
      <td><code>${esc(p.dose_state||'NOT_ATTEMPTED')}</code>
        <div class="meta">estado do leitor de <b>dose</b></div></td>
      <td>${contagem(p,'uses','LABEL_READ')}</td>
      <td><button class="ev" onclick="evIdx(${P.objects.indexOf(o)})">evidencia</button></td></tr>`;
    }).join('')}</tbody></table></div>`;
}
...
function viewCov() {
  ...
  <div class="block"><h3>Cobertura por etapa</h3>
    <div class="tw"><table><tbody>${Object.entries(c).map(([k,o]) =>
      k === 'PHI_COVERAGE'
        // X-05 · duas trocas erradas em sequencia. Desenhar "0 de 163, 0%" na
        // escala das coberturas medidas publicava um prototipo reprovado como
        // medicao; trocar por NOT_ATTEMPTED afirmava que nao houve tentativa,
        // e houve — bin/phi_extrair.py registra a corrida sobre 15 rotulos, e
        // as 166 fichas imprimiam "2 de 15" enquanto esta tela dizia "nunca
        // rodou". O estado certo nao e ignorancia nem cobertura: e prototipo
        // medido e NAO PUBLICADO.
        ? `<tr><td>${k.replace(/_/g,' ')}</td>
           <td><code>PROTOTYPE_NOT_SHIPPED</code></td>
           <td class="meta"><b>Nada de carencia e publicado nesta versao</b>, e nao por falta de
             tentativa: o extrator foi medido sobre <b>15</b> dos ${o.OF} rotulos e reprovou no
             proprio criterio &mdash; 2 com carencia lida, 8 com a secao localizada e nenhuma
             linha extraida, 5 sem secao, e a primeira linha de cada bloco contaminada pela coluna
             vizinha. Por isso esta linha nao tem barra: uma barra aqui seria cobertura, e nao ha
             cobertura a declarar. <b>As etichette trazem carencia; nos e que nao lemos.</b></td></tr>`
        : barra(k,o)).join('')}</tbody></table></div>
    <div class="meta" style="margin-top:9px">${esc(String(P.coverage_note).split('PHI_COVERAGE')[0].trim().replace(/[.,;]\s*$/,''))}.
      <div class="meta" style="margin-top:4px">A nota gravada no payload continua com uma frase
        sobre carencia que <b>nao e impressa aqui</b>: ela reapresenta como decisao um zero que e
        resultado de um prototipo reprovado, e um zero desses, escrito ao lado de coberturas
        medidas, le como medicao. O que vale sobre carencia esta na linha PHI da tabela
        acima.</div></div></div>
  <div style="margin:-4px 0 12px">
    <button class="ev" onclick="go('review')">ver os produtos por tras destes numeros</button>
    <button class="ev" onclick="go('produto')">abrir uma ficha</button>
    <button class="ev" onclick="go('timeline')">ver as ${P.versions.length} versoes do registro</button>
  </div>
...
function viewSearch() {
  ...
  if (!q) { $('#sres').innerHTML = `<div class="meta">digite um produto, registro, titular,
    substancia ativa, estado ou tipo de mudanca</div>
    <div class="lei">Esta demonstracao <b>nao busca por cultura nem por alvo</b>: essa camada foi
    retirada, e devolver uma lista de produtos para o nome de uma cultura seria responder com ela
    por outro caminho.</div>`; return; }
  // R-03 · a busca casava tambem por cultura e alvo dos pares de uso, entao
  // digitar "TABACCO" devolvia 29 produtos e o cliente lia aquilo como resposta
  // de uso autorizado. Medido: dos 29, ZERO casavam por nome, registro, titular,
  // ativo, estado, atividade ou formulacao — os 29 vinham so da camada retirada.
  ...
  // X-02 · tirar cultura e alvo do indice nao bastou. Nos objetos
  // NEEDS_HUMAN_REVIEW os campos BEFORE_VALUE/AFTER_VALUE SAO a dose retida:
  // digitar "280-600" devolvia 2 eventos nomeando LAMDEX EXTRA 008259, e "300"
  // devolvia 11 sobre 5 registros. E a associacao retida respondida por outra
  // porta — a busca confirmava um par que a demonstracao afirma nao exibir.
  const objs = P.objects.filter(o => {
    const campos = [o.PRODUCT_NAME, o.REGISTRATION_ID, o.CHANGE_TYPE];
    if (o.OBJECT_TYPE !== 'NEEDS_HUMAN_REVIEW') campos.push(o.BEFORE_VALUE, o.AFTER_VALUE);
  ...
  ${prods.length ? `<div class="tw"><table>
    <thead><tr><th>Produto</th><th>Registro</th><th>Titular</th><th>Ativos</th><th>Validade</th><th>Estado</th><th></th></tr></thead>
    <tbody>${prods.slice(0,80).map(p=>`<tr>
      ...
      <td class="meta">${val(p.status)}</td>
      <td><button class="ev" onclick="evProd('${p.reg}')">prova</button></td></tr>`).join('')}
    </tbody></table></div>
    ${prods.length>80?`<div class="meta"><b>mostrando 80 de ${prods.length}</b> &mdash;
      ${prods.length-80} produto(s) que casam com esta busca <b>nao estao nesta tela</b>.
      Nao aparecer aqui nao e prova de nao existir no registro.</div>`:''}`
    : '<div class="meta">nenhum produto</div>'}
  <h2>Eventos (${objs.length})</h2>
  ${glosaSe(objs)}
  ${objs.length ? objs.slice(0,40).map(cardObj).join('') +
    (objs.length>40?`<div class="meta"><b>mostrando 40 de ${objs.length}</b> &mdash;
      ${objs.length-40} evento(s) que casam com esta busca <b>nao estao nesta tela</b>.</div>`:'')
    : '<div class="meta">nenhum evento</div>'}`;
}
window.viewSearch = viewSearch;

// ---------------------------------------------------------------- roteador
const VIEWS = {home:viewHome, today:viewToday, produto:()=>viewProduto($('#psel').value),
               timeline:viewTimeline, cal:viewCal, action:viewAction,
