...
const ARPAE = {
  IDENTITY: {
    ...
    CAPTURES: {
      ...
      per: opc("(\\d{1,2})\\s+([a-z]+)\\s+(\\d{4})\\s*-\\s*(\\d{1,2})\\s+([a-z]+)\\s+(\\d{4})", 6, { FLAGS: "i" }),
    ...
    FACT_TIME_BASIS: "PERIODO_DE_OBSERVACAO_DECLARADO_NO_BOLETIM · 2.ª linha «<data> - <data>» do PDF",
...
const ARSAC = { IDENTITY: {
  ...
  FACT_TIME_BASIS: "VALIDADE_DECLARADA_NA_PAGINA_DA_EDICAO · «Settimana NN (…)»",
...
T("forma 1: «Settimana 39 (22/09/2026 – 29/09/2026)»", () => {
  ...
  assert.equal(id.FACT_TIME, "2026-09-22/2026-09-29");
  assert.ok(id.FACT_TIME_BASIS.includes("forma 1 de 2"));
});
...
T("forma 2: «Settimana 31 dal dal 20/07 al 04/08/2026» (o ano só no fim)", () => {
  ...
  assert.equal(id.FACT_TIME, "2026-07-20/2026-08-04");
  assert.ok(id.FACT_TIME_BASIS.includes("forma 2 de 2"));
});
...
T("lista de moldes vazia é contrato inválido", () => {
  assert.throws(() => conferirIdentidade("X", { ...structuredClone(ARSAC.IDENTITY), FACT_TIME: [] }), ContratoInvalido);
});
