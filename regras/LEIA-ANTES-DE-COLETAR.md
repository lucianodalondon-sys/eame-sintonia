...
- **Condutor da MICRO SOCIAL (LinkedIn + YouTube) — rodadas com travas, previsao do teto antes e prova-teto da noite depois** — Corre uma noite social por rodadas pela porta canonica (orquestrador -> scrap-colheita), com o Pedido montado em processo. Recusa antes da rede: sem autorizacao, lote 1 (PROVA-TETO-SOCIAL) por instalar, parada automatica anterior, robo a correr, Sala operacional por declarar, previsao do teto acima de 5 por dominio (gasto medido da noite + previsto), py sem yt_dlp numa rodada de YouTube, egresso != IT pelo portao de consenso. Por item pergunta ao portao de coleta no instante. Depois de cada rodada: egresso e PROVA-TETO sobre a noite inteira; != PASS escreve PARAR-MICRO-SOCIAL.flag. Le a Sala (so SELECT) com data e lugar, base e precisao, e chama o relatorio da micro web com --estado=.
